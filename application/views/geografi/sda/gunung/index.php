<div class="section">
	...
	<div class="page-header">
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active" aria-current="page">Gunung</li>
		</ol>
	</div>
	<!-- Page-header closed -->
		...
		<div class="col-md-12 col-lg-12">
			<div class="card" style="overflow:auto;">
				<div class="card-header">
					<div class="card-title">Gunung</div>
				...
				<div class="card-body">
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataGunung as $gunung): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('GEO','update')): ?>
										<button onclick="editModal(`<?= encrypt($gunung->id_gunung); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($gunung->id_gunung); ?>`,'<?= $gunung->nama_gunung; ?>')"
									...
									<td><?= $no++ ?></td>
									<td><?= $gunung->nama_satker ?></td>
									<td><?= $gunung->wilayah ?></td>
									<td><?= $gunung->nama_gunung ?></td>
									<td><?= $gunung->tinggi_gunung ?></td>
									<td><?= $gunung->status_gunung ?></td>
									<td><?= $gunung->keterangan ?></td>
									<td><?= $gunung->nama_pegawai ?></td>
									<td><?= $gunung->LastUpdated ?></td>
								</tr>
								<?php endforeach ?>
							</tbody>
						</table>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="name">Nama Gunung</label>
								<div class="col-md-9">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="high">Tinggi Gunung (Mdpl)</label>
								<div class="col-md-9">
									<input type="text" onkeypress="return isNumberKey(event)" id="" name="high" class="form-control">
									<div class="invalid-feedback warning-high"></div>
								</div>
							</div>
							<!-- <div class="form-group row">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label">Status Gunung</label>
								<div class="col-md-9">
									<select class="form-control" name="status" id="status" style="width: 100%;">
										<?php foreach($statusGunung as $status): ?>
											<option value="<?= $status->id_status_gunung ?>"><?= $status->nama ?></option>
										<?php endforeach?>
									</select>
									<div class="invalid-feedback warning-status"></div>
								</div>
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/geografi_gunung/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/geografi_gunungs/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>/geografi_gunung/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('input[name="satker"]').val(data.gunung.id_satker);
				$('input[name="name"]').val(data.gunung.nama_gunung);
				$('input[name="high"]').val(data.gunung.tinggi_gunung);
				$('input[name="notes"]').val(data.gunung.keterangan);
				$('input[name="flag_locationedit"]').val(data.gunung.flag_location);
				//$("select[name=satker] option[value="+data.gunung.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.gunung.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.gunung.id_provinsi+"]").attr('selected','selected');
				//$("select[name=status] option[value="+data.gunung.id_status_gunung+"]").attr('selected','selected');

				$("#satkerEdit").val(data.gunung.id_satker);
				$("#provinsiEdit").val(data.gunung.id_provinsi);
				$("#statusEdit").val(data.gunung.id_status_gunung);
				...
				if(data.gunung.flag_location == 'prov')
				{
					getProvinsi(data.gunung.id_provinsi)
					getKabupaten(data.gunung.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.gunung.flag_location == 'kab')
				{
					getProvinsi(data.gunung.id_provinsi)
					getKabupaten(data.gunung.id_provinsi,data.gunung.id_kabupaten)
					getKecamatan(data.gunung.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.gunung.flag_location == 'kec')
				{
					getProvinsi(data.gunung.id_provinsi)
					getKabupaten(data.gunung.id_provinsi,data.gunung.id_kabupaten)
					getKecamatan(data.gunung.id_kabupaten,data.gunung.id_kecamatan)
					getKelurahan(data.gunung.id_kecamatan,0)
				}
				else if(data.gunung.flag_location == 'kel')
				{
					getProvinsi(data.gunung.id_provinsi)
					getKabupaten(data.gunung.id_provinsi,data.gunung.id_kabupaten)
					getKecamatan(data.gunung.id_kabupaten,data.gunung.id_kecamatan)
					getKelurahan(data.gunung.id_kecamatan,data.gunung.id_kelurahan)
				}
				else
				{
					getProvinsi(data.gunung.id_provinsi)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>/geografi_gunung/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
