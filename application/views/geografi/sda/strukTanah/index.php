<div class="section">
	...
	<div class="page-header">
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active" aria-current="page">Struktur Tanah</li>
		</ol>
	</div>
	<!-- Page-header closed -->
		...
		<div class="col-md-12 col-lg-12">
			<div class="card" style="overflow:auto;">
				<div class="card-header">
					<div class="card-title">Struktur Tanah</div>
				...
				<div class="card-body">
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataTanah as $tanah): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('GEO','update')): ?>
										<button onclick="editModal(`<?= encrypt($tanah->id_struktur_tanah); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($tanah->id_struktur_tanah); ?>`,'Struktur Tanah')"
									...
									<td><?= $no++ ?></td>
									<td><?= $tanah->nama_satker ?></td>
									<td><?= $tanah->wilayah ?></td>
									<td><?= $tanah->jenis_tanah ?></td>
									<td><?= $tanah->kemiringan ?></td>
									<td><?= $tanah->pemanfaatan ?></td>
									<td><?= $tanah->keterangan ?></td>
									<td><?= $tanah->nama_pegawai ?></td>
									<td><?= $tanah->LastUpdated ?></td>
								</tr>
								<?php endforeach ?>
							</tbody>
						</table>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<!-- <div class="form-group row">
								<label class="col-md-3 col-form-label" for="type">Jenis Tanah</label>
								<div class="col-md-9">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="material">Jenis Tanah</label>
								<div class="col-md-9">
									<select class="form-control" name="type" id="type" style="width: 100%;">
										<?php foreach($jenisTanah as $jenis): ?>
											<option value="<?= $jenis->id_jenis_tanah ?>"><?= $jenis->nama ?></option>
										<?php endforeach?>
									</select>
									<div class="invalid-feedback warning-type"></div>
								</div>
							...
							<!-- <div class="form-group row">
								<label class="col-md-3 col-form-label" for="slope">Kemiringan</label>
								<div class="col-md-9">
									<input type="text" id="" name="slope" class="form-control">
									<div class="invalid-feedback warning-slope"></div>
								</div>
							</div> -->
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="slope">Kemiringan</label>
								<div class="col-md-9">
									<select class="form-control" name="slope" id="slope" style="width: 100%;">
										<option value="Datar (0-8 %)">Datar (0-8 %)</option>
										<option value="Landai (8-15 %)">Landai (8-15 %)</option>
										<option value="Sedikit Curam (15-25 %)">Sedikit Curam (15-25 %)</option>
					...
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="slope">Kemiringan</label>
								<div class="col-md-9">
									<select class="form-control" name="slope" id="slopeEdit" style="width: 100%;">
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/geografi_strukTanah/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>geografi_strukTanahs/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>geografi_strukTanah/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('input[name="satker"]').val(data.tanah.id_satker);
				//$('input[name="slope"]').val(data.tanah.kemiringan);
				$('input[name="utilization"]').val(data.tanah.pemanfaatan);
				$('input[name="notes"]').val(data.tanah.keterangan);
				$('input[name="flag_locationedit"]').val(data.tanah.flag_location);
				//$("select[name=satker] option[value="+data.tanah.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.tanah.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.tanah.id_provinsi+"]").attr('selected','selected');
				//$("select[name=type] option[value="+data.tanah.id_jenis_tanah+"]").attr('selected','selected');

				$("#satkerEdit").val(data.tanah.id_satker);
				$("#provinsiEdit").val(data.tanah.id_provinsi);
				$("#typeEdit").val(data.tanah.id_jenis_tanah);
				$("#slopeEdit").val(data.tanah.kemiringan);
				...
				$("#slopeEdit").trigger('change');

				if(data.tanah.flag_location == 'prov')
				{
					getProvinsi(data.tanah.id_provinsi)
					getKabupaten(data.tanah.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.tanah.flag_location == 'kab')
				{
					getProvinsi(data.tanah.id_provinsi)
					getKabupaten(data.tanah.id_provinsi,data.tanah.id_kabupaten)
					getKecamatan(data.tanah.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.tanah.flag_location == 'kec')
				{
					getProvinsi(data.tanah.id_provinsi)
					getKabupaten(data.tanah.id_provinsi,data.tanah.id_kabupaten)
					getKecamatan(data.tanah.id_kabupaten,data.tanah.id_kecamatan)
					getKelurahan(data.tanah.id_kecamatan,0)
				}
				else if(data.tanah.flag_location == 'kel')
				{
					getProvinsi(data.tanah.id_provinsi)
					getKabupaten(data.tanah.id_provinsi,data.tanah.id_kabupaten)
					getKecamatan(data.tanah.id_kabupaten,data.tanah.id_kecamatan)
					getKelurahan(data.tanah.id_kecamatan,data.tanah.id_kelurahan)
				}
				else
				{
					getProvinsi(data.tanah.id_provinsi)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>geografi_strukTanah/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
