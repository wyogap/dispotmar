<div class="section">
	...
	<div class="page-header">
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active" aria-current="page">Curah Hujan</li>
		</ol>
	</div>
	<!-- Page-header closed -->
			...
			<div class="card" style="overflow:auto;">
				...
				<div class="card-body">
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataHujan as $hujan): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('GEO','update')): ?>
										<button onclick="editModal(`<?= encrypt($hujan->id_curah_hujan); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($hujan->id_curah_hujan); ?>`,'Hujan')"
									...
									<td><?= $no++ ?></td>
									<td><?= $hujan->nama_satker ?></td>
									<td><?= $hujan->wilayah ?></td>
									<td><?= $hujan->suhu_min ?></td>
									<td><?= $hujan->suhu_max ?></td>
									<td><?= $hujan->kelembaban_udara ?></td>
									<td><?= $hujan->musim_hujan ?></td>
									<td><?= $hujan->curah_hujan ?></td>
									<td><?= $hujan->keterangan ?></td>
									<td><?= $hujan->nama_pegawai ?></td>
									<td><?= $hujan->LastUpdated ?></td>
								</tr>
								<?php endforeach ?>
							</tbody>
						</table>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="min">Suhu Min (°C)</label>
								<div class="col-md-9">
									<input type="text" onkeypress="return isNumberKey(event)" id="" name="min" class="form-control">
									<div class="invalid-feedback warning-min"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="max">Suhu Max (°C)</label>
								<div class="col-md-9">
									<input type="text" onkeypress="return isNumberKey(event)" id="" name="max" class="form-control">
									<div class="invalid-feedback warning-max"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="humidity">Kelembapan Udara</label>
								<div class="col-md-9">
									<input type="text" onkeypress="return isNumberKey(event)" id="" name="humidity" class="form-control">
									<div class="invalid-feedback warning-humidity"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="rainy">Musim Hujan (Bulan/Tahun)</label>
								<div class="col-md-9">
									<input type="text" id="" name="rainy" class="form-control">
									<div class="invalid-feedback warning-rainy"></div>
								</div>
							</div>
							<!-- <div class="form-group row">
								<label class="col-md-3 col-form-label" for="category">Kategori Curah Hujan</label>
								<div class="col-md-9">
									<select class="form-control" name="category">
										<option value="Rendah">Rendah (0-100 mm)</option>
										<option value="Sedang">Menengah (100-300 mm)</option>
										<option value="Tinggi">Tinggi (300-500 mm)</option>
									</select>
									<div class="invalid-feedback warning-category"></div>
								</div>
							</div>	 -->
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="rainfall">Curah Hujan</label>
								<div class="col-md-9">
									<!-- <input type="text" id="" name="rainfall" class="form-control"> -->
									<select class="form-control" name="rainfall" id="rainfall" style="width: 100%;">
										<option value="Rendah">Rendah (0-100 mm)</option>
										<option value="Menengah">Menengah (100-300 mm)</option>
										<option value="Tinggi">Tinggi (300-500 mm)</option>
									</select>
									<div class="invalid-feedback warning-rainfall"></div>
								</div>
							</div>
							<div class="form-group row">
					...
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								...
								<div class="col-md-9">
									<!-- <input type="text" id="" name="rainfall" class="form-control"> -->
									<select class="form-control" name="rainfall" id="rainfallEdit" style="width: 100%;">
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/geografi_curahHujan/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>geografi_curahHujans/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>geografi_curahHujan/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('input[name="satker"]').val(data.hujan.id_satker);
				$('input[name="min"]').val(data.hujan.suhu_min);
				$('input[name="max"]').val(data.hujan.suhu_max);
				$('input[name="humidity"]').val(data.hujan.kelembaban_udara);
				$('input[name="rainy"]').val(data.hujan.musim_hujan);
				// $('input[name="rainfall"]').val(data.hujan.curah_hujan);
				$('select[name=rainfall]').find('option:selected').removeAttr('selected');
				$('input[name="notes"]').val(data.hujan.keterangan);
				$('input[name="flag_locationedit"]').val(data.hujan.flag_location);
				//$("select[name=satker] option[value="+data.hujan.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.hujan.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.hujan.id_provinsi+"]").attr('selected','selected');
				// $("select[name=category] option[value="+data.hujan.kategori_curah_hujan+"]").attr('selected','selected');
				//$("select[name=rainfall] option[value="+data.hujan.curah_hujan+"]").attr('selected','selected');

				$("#satkerEdit").val(data.hujan.id_satker);
				$("#provinsiEdit").val(data.hujan.id_provinsi);
				$("#rainfallEdit").val(data.hujan.curah_hujan);
				...
				$("#rainfallEdit").trigger('change');

				if(data.hujan.flag_location == 'prov')
				{
					getProvinsi(data.hujan.id_provinsi)
					getKabupaten(data.hujan.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.hujan.flag_location == 'kab')
				{
					getProvinsi(data.hujan.id_provinsi)
					getKabupaten(data.hujan.id_provinsi,data.hujan.id_kabupaten)
					getKecamatan(data.hujan.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.hujan.flag_location == 'kec')
				{
					getProvinsi(data.hujan.id_provinsi)
					getKabupaten(data.hujan.id_provinsi,data.hujan.id_kabupaten)
					getKecamatan(data.hujan.id_kabupaten,data.hujan.id_kecamatan)
					getKelurahan(data.hujan.id_kecamatan,0)
				}
				else if(data.hujan.flag_location == 'kel')
				{
					getProvinsi(data.hujan.id_provinsi)
					getKabupaten(data.hujan.id_provinsi,data.hujan.id_kabupaten)
					getKecamatan(data.hujan.id_kabupaten,data.hujan.id_kecamatan)
					getKelurahan(data.hujan.id_kecamatan,data.hujan.id_kelurahan)
				}
				else
				{
					getProvinsi(data.hujan.id_provinsi)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>geografi_curahHujan/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
