<div class="section">
	...
	<div class="page-header">
		...
		<li class="breadcrumb-item"><a href="#"><i class="ti-package mr-1"></i> Geografi</a></li>
			...
			<li class="breadcrumb-item active" aria-current="page">Hutan</li>
		</ol>
	</div>
	<!-- Page-header closed -->
		...
		<div class="col-md-12 col-lg-12">
			<div class="card" style="overflow:auto;">
				<div class="card-header">
					<div class="card-title">Hutan</div>
				...
				<div class="card-body">
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataHutan as $hutan): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('GEO','update')): ?>
										<button onclick="editModal(`<?= encrypt($hutan->id_hutan); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($hutan->id_hutan); ?>`,'<?= $hutan->jenis_tanaman; ?>')"
									...
									<td><?= $no++ ?></td>
									<td><?= $hutan->nama_satker ?></td>
									<td><?= $hutan->wilayah ?></td>
									<td><?= $hutan->jenis_tanaman ?></td>
									<td><?= $hutan->luas_hutan ?></td>
									<td><?= $hutan->status_hutan ?></td>
									<td><?= $hutan->keterangan ?></td>
									<td><?= $hutan->nama_pegawai ?></td>
									<td><?= $hutan->LastUpdated ?></td>
								</tr>
								<?php endforeach ?>
							</tbody>
						</table>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label">Jenis Tanaman</label>
								<div class="col-md-9">
									<select class="form-control" name="type" id="type" style="width: 100%;">
										<?php foreach($jenisTanaman as $jenis): ?>
											<option value="<?= $jenis->id_jenis_tanaman_hutan ?>"><?= $jenis->nama ?></option>
										<?php endforeach?>
									</select>
									<div class="invalid-feedback warning-type"></div>
								</div>
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="area">Luas Hutan (Ha)</label>
								<div class="col-md-9">
									<input type="text" id="" name="area" class="form-control" onkeypress="return isNumberKey(event)">
									<div class="invalid-feedback warning-area"></div>
								</div>
							</div>
							<!-- <div class="form-group row">
								<label class="col-md-3 col-form-label" for="status">Status Hutan</label>
								<div class="col-md-9">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label">Status Hutan</label>
								<div class="col-md-9">
									<select class="form-control" name="status" id="status" style="width: 100%;">
										<?php foreach($statusHutan as $status): ?>
											<option value="<?= $status->id_status_hutan ?>"><?= $status->nama ?></option>
										<?php endforeach?>
									</select>
									<div class="invalid-feedback warning-status"></div>
								</div>
					...
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label">Status Hutan</label>
								<div class="col-md-9">
									<select class="form-control" name="status" id="statusEdit" style="width: 100%;">
										<?php foreach($statusHutan as $status): ?>
											<option value="<?= $status->id_status_hutan ?>"><?= $status->nama ?></option>
										<?php endforeach?>
									</select>
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/geografi_hutan/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/geografi_hutans/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>/geografi_hutan/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('input[name="satker"]').val(data.hutan.id_satker);
				$('input[name="area"]').val(data.hutan.luas_hutan);
				$('input[name="notes"]').val(data.hutan.keterangan);
				$('input[name="flag_locationedit"]').val(data.hutan.flag_location);
				//$("select[name=satker] option[value="+data.hutan.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.hutan.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.hutan.id_provinsi+"]").attr('selected','selected');
				//$("select[name=type] option[value="+data.hutan.id_jenis_tanaman_hutan+"]").attr('selected','selected');
				//$("select[name=status] option[value="+data.hutan.id_status_hutan+"]").attr('selected','selected');
				$("#satkerEdit").val(data.hutan.id_satker);
				$("#provinsiEdit").val(data.hutan.id_provinsi);
				$("#typeEdit").val(data.hutan.id_jenis_tanaman_hutan);
				$("#statusEdit").val(data.hutan.id_status_hutan);
				...
				$("#statusEdit").trigger('change');

				if(data.hutan.flag_location == 'prov')
				{
					getProvinsi(data.hutan.id_provinsi)
					getKabupaten(data.hutan.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.hutan.flag_location == 'kab')
				{
					getProvinsi(data.hutan.id_provinsi)
					getKabupaten(data.hutan.id_provinsi,data.hutan.id_kabupaten)
					getKecamatan(data.hutan.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.hutan.flag_location == 'kec')
				{
					getProvinsi(data.hutan.id_provinsi)
					getKabupaten(data.hutan.id_provinsi,data.hutan.id_kabupaten)
					getKecamatan(data.hutan.id_kabupaten,data.hutan.id_kecamatan)
					getKelurahan(data.hutan.id_kecamatan,0)
				}
				else if(data.hutan.flag_location == 'kel')
				{
					getProvinsi(data.hutan.id_provinsi)
					getKabupaten(data.hutan.id_provinsi,data.hutan.id_kabupaten)
					getKecamatan(data.hutan.id_kabupaten,data.hutan.id_kecamatan)
					getKelurahan(data.hutan.id_kecamatan,data.hutan.id_kelurahan)
				}
				else
				{
					getProvinsi(data.hutan.id_provinsi)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>/geografi_hutan/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
