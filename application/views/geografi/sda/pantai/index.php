<div class="section">
	...
	<div class="page-header">
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active" aria-current="page">Pantai</li>
		</ol>
	</div>
	<!-- Page-header closed -->
				...
				<div class="card-body">
					...
					<div class="table-responsive">
						<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataPantai as $pantai): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('GEO','update')): ?>
										<button onclick="editModal(`<?= encrypt($pantai->id_pantai); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($pantai->id_pantai); ?>`,'<?= $pantai->nama_pantai; ?>')"
									...
									<td><?= $no++ ?></td>
									<td><?= $pantai->nama_satker ?></td>
									<td><?= $pantai->wilayah ?></td>
									<td><?= $pantai->nama_pantai ?></td>
									<td><?= $pantai->jenis_pantai ?></td>
									<td><?= $pantai->panjang_pantai ?></td>
									<td><?= ucwords(str_replace('_',' ',$pantai->material_dasar_pantai)) ?></td>
									<td><?= $pantai->ciri_khusus ?></td>
									<td><?= $pantai->keterangan ?></td>
									<td><?= $pantai->nama_pegawai ?></td>
									<td><?= $pantai->LastUpdated ?></td>
								</tr>
								<?php endforeach ?>
							</tbody>
						</table>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="name">Nama Pantai</label>
								<div class="col-md-9">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label">Jenis Pantai </label>
								<div class="col-md-9">
									<select class="form-control" name="type" id="type" style="width: 100%;">
										<?php foreach($jenisPantai as $jenis): ?>
											<option value="<?= $jenis->id_jenis_pantai ?>"><?= $jenis->nama ?></option>
										<?php endforeach?>
									</select>
							...
							<!-- <div class="form-group row">
								<label class="col-md-3 col-form-label" for="type">Jenis Pantai</label>
								<div class="col-md-9">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="long">Panjang Pantai (Km)</label>
								<div class="col-md-9">
									<input type="text" onkeypress="return isNumberKey(event)" id="" name="long" class="form-control">
									<div class="invalid-feedback warning-long"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="material">Material Dasar Pantai</label>
								<div class="col-md-9">
									<select class="form-control" name="material" id="material" style="width: 100%;">
										<option value="batu_kasar" selected>Batu Kasar</option>
										<option value="kerikil">Kerikil</option>
										<option value="pasir">Pasir</option>
										<option value="lumpur">Lumpur</option>
									</select>
									<div class="invalid-feedback warning-material"></div>
								</div>
							</div>	
							<!-- <div class="form-group row">
								<label class="col-md-3 col-form-label" for="material">Material Dasar Pantai</label>
								<div class="col-md-9">
									<input type="text" id="" name="material" class="form-control">
									<div class="invalid-feedback warning-material"></div>
								</div>
							</div> -->
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="ciri">Ciri Khusus</label>
								<div class="col-md-9">
									<input type="text" id="" name="ciri" class="form-control">
									<div class="invalid-feedback warning-ciri"></div>
								</div>
							</div>
							<div class="form-group row">
					...
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="material">Material Dasar Pantai</label>
								<div class="col-md-9">
									<select class="form-control" name="material" id="materialEdit" style="width: 100%;">
										<option value="">Pilih</option>
										<option value="batu_kasar">Batu Kasar</option>
										<option value="kerikil">Kerikil</option>
										<option value="pasir">Pasir</option>
										<option value="lumpur">Lumpur</option>
									</select>
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/geografi_pantai/store",
				dataType: "json",
				data: $(this).serialize(),
	...
	function editModal(id) {
		...
		$.ajax({
			...
			success: function (data) {
				...
				$('select[name=material]').find('option:selected').removeAttr('selected');
				$('input[name="id"]').val(id);
				$('input[name="name"]').val(data.pantai.nama_pantai);
				$('input[name="satker"]').val(data.pantai.id_satker);
				$('input[name="long"]').val(data.pantai.panjang_pantai);
				$('input[name="ciri"]').val(data.pantai.ciri_khusus);
				$('input[name="notes"]').val(data.pantai.keterangan);
				$('input[name="flag_locationedit"]').val(data.pantai.flag_location);
				//$("select[name=satker] option[value="+data.pantai.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.pantai.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.pantai.id_provinsi+"]").attr('selected','selected');
				//$("select[name=type] option[value="+data.pantai.id_jenis_pantai+"]").attr('selected','selected');
				//$("select[name=material] option[value="+data.pantai.material_dasar_pantai+"]").attr('selected','selected');
				$("#satkerEdit").val(data.pantai.id_satker);
				$("#provinsiEdit").val(data.pantai.id_provinsi);
				$("#typeEdit").val(data.pantai.id_jenis_pantai);
				$("#materialEdit").val(data.pantai.material_dasar_pantai);
				...
				$("#materialEdit").trigger('change');

				if(data.pantai.flag_location == 'prov')
				{
					getProvinsi(data.pantai.id_provinsi)
					getKabupaten(data.pantai.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.pantai.flag_location == 'kab')
				{
					getProvinsi(data.pantai.id_provinsi)
					getKabupaten(data.pantai.id_provinsi,data.pantai.id_kabupaten)
					getKecamatan(data.pantai.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.pantai.flag_location == 'kec')
				{
					getProvinsi(data.pantai.id_provinsi)
					getKabupaten(data.pantai.id_provinsi,data.pantai.id_kabupaten)
					getKecamatan(data.pantai.id_kabupaten,data.pantai.id_kecamatan)
					getKelurahan(data.pantai.id_kecamatan,0)
				}
				else if(data.pantai.flag_location == 'kel')
				{
					getProvinsi(data.pantai.id_provinsi)
					getKabupaten(data.pantai.id_provinsi,data.pantai.id_kabupaten)
					getKecamatan(data.pantai.id_kabupaten,data.pantai.id_kecamatan)
					getKelurahan(data.pantai.id_kecamatan,data.pantai.id_kelurahan)
				}
				else
				{
					getProvinsi(data.pantai.id_provinsi)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', 'geografi_pantai/' + id + '/delete');
		$('#deleteModal').modal();
	}
	...
	function getdataSENSOR_detail(tokenvalue) {
    	...
    	$.ajax({
			url: 'https://cariudata1.dropadi.com/api/sensor-data?timestamp=2021-01-05T21:06:00%2B07:00,2021-01-05T21:13:00%2B07:00',
        	type: 'GET',
        	dataType: 'json',
        	headers: {
	...
	function insertdataSENSOR(id,code,category,group_th,group_label,node_id,node_label,timestamp,unit,value,csrfvalue)
	{
