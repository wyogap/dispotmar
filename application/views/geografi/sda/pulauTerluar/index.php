<div class="section">
	...
	<div class="page-header">
		...
		<li class="breadcrumb-item"><a href="#"><i class="ti-package mr-1"></i> Geografi</a></li>
			...
			<li class="breadcrumb-item active" aria-current="page">Pulau Terluar</li>
		</ol>
	</div>
	<!-- Page-header closed -->
		...
		<div class="col-md-12 col-lg-12">
			<div class="card" style="overflow:auto;">
				<div class="card-header">
					<div class="card-title">Pulau Terluar</div>
				...
				<div class="card-body">
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataPulau as $pulau): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('GEO','update')): ?>
										<button onclick="editModal(`<?= encrypt($pulau->id_pulau_terluar); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($pulau->id_pulau_terluar); ?>`,'<?= $pulau->nama_pulau ?>')"
									...
									<td><?= $no++ ?></td>
									<td><?= $pulau->nama_satker ?></td>
									<td><?= $pulau->wilayah ?></td>
									<td><?= $pulau->nama_pulau ?></td>
									<td><?= $pulau->luas_pulau ?></td>
									<td><?= $pulau->jumlah_penduduk ?></td>
									<td><?= $pulau->jarak_pulau_utama ?></td>
									<td><?= $pulau->transportasi ?></td>
									<td><?= $pulau->keterangan ?></td>
									<td><?= $pulau->nama_pegawai ?></td>
									<td><?= $pulau->LastUpdated ?></td>
								</tr>
								<?php endforeach ?>
							</tbody>
						</table>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="name">Nama Pulau</label>
								<div class="col-md-9">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="area">Luas Pulau (Ha)</label>
								<div class="col-md-9">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="population">Jumlah Penduduk (orang)</label>
								<div class="col-md-9">
									<input type="number" id="" name="population" class="form-control">
									<div class="invalid-feedback warning-population"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="range">Jarak Pulau Utama (km)</label>
								<div class="col-md-9">
							...
							<!-- <div class="form-group row">
								<label class="col-md-3 col-form-label" for="transportation">Transportasi</label>
								<div class="col-md-9">
									<input type="text" id="" name="transportation" class="form-control">
									<div class="invalid-feedback warning-transportation"></div>
								</div>
							</div> -->
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="rainfall">Transportasi</label>
								<div class="col-md-9">
									<select class="form-control" name="transportation" id="transportation" style="width: 100%;">
					...
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="rainfall">Transportasi</label>
								<div class="col-md-9">
									<select class="form-control" name="transportation" id="transportationEdit" style="width: 100%;">
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/geografi_pulauTerluar/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>geografi_pulauTerluars/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>geografi_pulauTerluar/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('input[name="satker"]').val(data.pulau.id_satker);
				$('input[name="name"]').val(data.pulau.nama_pulau);
				$('input[name="area"]').val(data.pulau.luas_pulau);
				$('input[name="population"]').val(data.pulau.jumlah_penduduk);
				$('input[name="range"]').val(data.pulau.jarak_pulau_utama);
				//$('input[name="transportation"]').val(data.pulau.transportasi);
				$('input[name="notes"]').val(data.pulau.keterangan);
				$('input[name="flag_locationedit"]').val(data.pulau.flag_location);
				//$("select[name=satker] option[value="+data.pulau.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.pulau.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.pulau.id_provinsi+"]").attr('selected','selected');

				$("#satkerEdit").val(data.pulau.id_satker);
				$("#provinsiEdit").val(data.pulau.id_provinsi);
				$("#transportationEdit").val(data.pulau.transportasi);
				...
				$("#transportationEdit").trigger('change');

				if(data.pulau.flag_location == 'prov')
				{
					getProvinsi(data.pulau.id_provinsi)
					getKabupaten(data.pulau.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.pulau.flag_location == 'kab')
				{
					getProvinsi(data.pulau.id_provinsi)
					getKabupaten(data.pulau.id_provinsi,data.pulau.id_kabupaten)
					getKecamatan(data.pulau.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.pulau.flag_location == 'kec')
				{
					getProvinsi(data.pulau.id_provinsi)
					getKabupaten(data.pulau.id_provinsi,data.pulau.id_kabupaten)
					getKecamatan(data.pulau.id_kabupaten,data.pulau.id_kecamatan)
					getKelurahan(data.pulau.id_kecamatan,0)
				}
				else if(data.pulau.flag_location == 'kel')
				{
					getProvinsi(data.pulau.id_provinsi)
					getKabupaten(data.pulau.id_provinsi,data.pulau.id_kabupaten)
					getKecamatan(data.pulau.id_kabupaten,data.pulau.id_kecamatan)
					getKelurahan(data.pulau.id_kecamatan,data.pulau.id_kelurahan)
				}
				else
				{
					getProvinsi(data.pulau.id_provinsi)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>geografi_pulauTerluar/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
