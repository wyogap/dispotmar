<div class="section">
	...
	<div class="page-header">
		...
		<li class="breadcrumb-item"><a href="#"><i class="ti-package mr-1"></i> Geografi</a></li>
			...
			<li class="breadcrumb-item active" aria-current="page">Sungai</li>
		</ol>
	</div>
	<!-- Page-header closed -->
		...
		<div class="col-md-12 col-lg-12">
			<div class="card" style="overflow:auto;">
				<div class="card-header">
					<div class="card-title">Sungai</div>
				...
				<div class="card-body">
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataSungai as $sungai): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('GEO','update')): ?>
										<button onclick="editModal(`<?= encrypt($sungai->id_sungai); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($sungai->id_sungai); ?>`,'<?= $sungai->nama_sungai ?>')"
									...
									<td><?= $sungai->sumber_sungai ?></td>
									<td><?= $sungai->anak_sungai ?></td>
									<td><?= $sungai->pemanfaatan_sungai ?></td>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="name">Nama Sungai</label>
								<div class="col-md-9">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="width">Lebar (m)</label>
								<div class="col-md-9">
									<input type="text" onkeypress="return isNumberKey(event)" id="" name="width" class="form-control">
									<div class="invalid-feedback warning-width"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="long">Panjang (Km)</label>
								<div class="col-md-9">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="sources">Sumber Sungai</label>
								<div class="col-md-9">
									<input type="text" id="" name="sources" class="form-control">
									<div class="invalid-feedback warning-sources"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="child">Anak Sungai</label>
								<div class="col-md-9">
									<input type="text" id="" name="child" class="form-control">
									<div class="invalid-feedback warning-child"></div>
								</div>
							</div>
							<!-- <div class="form-group row">
								<label class="col-md-3 col-form-label" for="utilization">Pemanfaatan</label>
								<div class="col-md-9">
									<input type="text" id="" name="utilization" class="form-control">
									<div class="invalid-feedback warning-utilization"></div>
								</div>
							</div> -->
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="utilization">Pemanfaatan</label>
								<div class="col-md-9">
									<select class="form-control" name="utilization" id="utilization" style="width: 100%;">
										<?php foreach($pemanfaatan as $manfaat): ?>
											<option value="<?= $manfaat->id_pemanfaatan_sungai ?>"><?= $manfaat->nama ?></option>
										<?php endforeach?>
									</select>
									<div class="invalid-feedback warning-utilization"></div>
								</div>
					...
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="utilization">Pemanfaatan</label>
								<div class="col-md-9">
									<select class="form-control" name="utilization" id="utilizationEdit" style="width: 100%;">
										<?php foreach($pemanfaatan as $manfaat): ?>
											<option value="<?= $manfaat->id_pemanfaatan_sungai ?>"><?= $manfaat->nama ?></option>
										<?php endforeach?>
									</select>
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/geografi_sungai/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>geografi_sungais/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>geografi_sungai/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('select[name=utilization]').find('option:selected').removeAttr('selected');
				$('input[name="id"]').val(id);
				$('input[name="satker"]').val(data.sungai.id_satker);
				$('input[name="name"]').val(data.sungai.nama_sungai);
				$('input[name="width"]').val(data.sungai.lebar);
				$('input[name="long"]').val(data.sungai.panjang);
				$('input[name="sources"]').val(data.sungai.sumber_sungai);
				$('input[name="child"]').val(data.sungai.anak_sungai);
				$('input[name="notes"]').val(data.sungai.keterangan);
				$('input[name="flag_locationedit"]').val(data.sungai.flag_location);
				//$("select[name=satker] option[value="+data.sungai.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.sungai.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.sungai.id_provinsi+"]").attr('selected','selected');
				//$("select[name=utilization] option[value="+data.sungai.id_pemanfaatan_sungai+"]").attr('selected','selected');

				$("#satkerEdit").val(data.sungai.id_satker);
				$("#provinsiEdit").val(data.sungai.id_provinsi);
				$("#utilizationEdit").val(data.sungai.id_pemanfaatan_sungai);
				...
				$("#utilizationEdit").trigger('change');

				if(data.sungai.flag_location == 'prov')
				{
					getProvinsi(data.sungai.id_provinsi)
					getKabupaten(data.sungai.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.sungai.flag_location == 'kab')
				{
					getProvinsi(data.sungai.id_provinsi)
					getKabupaten(data.sungai.id_provinsi,data.sungai.id_kabupaten)
					getKecamatan(data.sungai.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.sungai.flag_location == 'kec')
				{
					getProvinsi(data.sungai.id_provinsi)
					getKabupaten(data.sungai.id_provinsi,data.sungai.id_kabupaten)
					getKecamatan(data.sungai.id_kabupaten,data.sungai.id_kecamatan)
					getKelurahan(data.sungai.id_kecamatan,0)
				}
				else if(data.sungai.flag_location == 'kel')
				{
					getProvinsi(data.sungai.id_provinsi)
					getKabupaten(data.sungai.id_provinsi,data.sungai.id_kabupaten)
					getKecamatan(data.sungai.id_kabupaten,data.sungai.id_kecamatan)
					getKelurahan(data.sungai.id_kecamatan,data.sungai.id_kelurahan)
				}
				else
				{
					getProvinsi(data.sungai.id_provinsi)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>geografi_sungai/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
