<div class="section">
	...
	<div class="page-header">
		...
		<li class="breadcrumb-item"><a href="#"><i class="ti-package mr-1"></i> Geografi</a></li>
			...
			<li class="breadcrumb-item active" aria-current="page">Sumber Air</li>
		</ol>
	</div>
	<!-- Page-header closed -->
		...
		<div class="col-md-12 col-lg-12">
			<div class="card" style="overflow:auto;">
				<div class="card-header">
					<div class="card-title">Sumber Air</div>
				...
				<div class="card-body">
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataAir as $air): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('GEO','update')): ?>
										<button onclick="editModal(`<?= encrypt($air->id_sumber_air); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($air->id_sumber_air); ?>`,'Sumber Air')"
									...
									<td><?= $no++ ?></td>
									<td><?= $air->nama_satker ?></td>
									<td><?= $air->wilayah ?></td>
									<td><?= $air->jenis_sumberair ?></td>
									<td><?= $air->debit_air ?></td>
									<td><?= $air->kondisi_air ?></td>
									<td><?= $air->keterangan ?></td>
									<td><?= $air->nama_pegawai ?></td>
									<td><?= $air->LastUpdated ?></td>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<!-- <div class="form-group row">
								<label class="col-md-3 col-form-label" for="sources">Sumber Air</label>
								<div class="col-md-9">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="material">Jenis Sumber Air</label>
								<div class="col-md-9">
									<select class="form-control" name="sources" id="sources" style="width: 100%;">
										<?php foreach($jenisSumber as $jenis): ?>
											<option value="<?= $jenis->id_jenis_sumberair ?>"><?= $jenis->nama ?></option>
										<?php endforeach?>
									</select>
									<div class="invalid-feedback warning-sources"></div>
								</div>
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="volume">Debit Air</label>
								<div class="col-md-9">
									<input type="text" onkeypress="return isNumberKey(event)" id="" name="volume" class="form-control">
									<div class="invalid-feedback warning-volume"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="condition">Kondisi Air</label>
								<div class="col-md-9">
									<input type="text" id="" name="condition" class="form-control">
									<div class="invalid-feedback warning-condition"></div>
								</div>
							</div>
							<div class="form-group row">
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/geografi_sumberAir/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>geografi_sumberAirs/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>geografi_sumberAir/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('input[name="satker"]').val(data.air.id_satker);
				$('input[name="sources"]').val(data.air.sumber_air);
				$('input[name="volume"]').val(data.air.debit_air);
				$('input[name="condition"]').val(data.air.kondisi_air);
				$('input[name="notes"]').val(data.air.keterangan);
				$('input[name="flag_locationedit"]').val(data.air.flag_location);
				//$("select[name=satker] option[value="+data.air.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.air.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.air.id_provinsi+"]").attr('selected','selected');
				//$("select[name=sources] option[value="+data.air.id_jenis_sumberair+"]").attr('selected','selected');

				$("#satkerEdit").val(data.air.id_satker);
				$("#provinsiEdit").val(data.air.id_provinsi);
				$("#sourcesEdit").val(data.air.id_jenis_sumberair);
				...
				if(data.air.flag_location == 'prov')
				{
					getProvinsi(data.air.id_provinsi)
					getKabupaten(data.air.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.air.flag_location == 'kab')
				{
					getProvinsi(data.air.id_provinsi)
					getKabupaten(data.air.id_provinsi,data.air.id_kabupaten)
					getKecamatan(data.air.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.air.flag_location == 'kec')
				{
					getProvinsi(data.air.id_provinsi)
					getKabupaten(data.air.id_provinsi,data.air.id_kabupaten)
					getKecamatan(data.air.id_kabupaten,data.air.id_kecamatan)
					getKelurahan(data.air.id_kecamatan,0)
				}
				else if(data.air.flag_location == 'kel')
				{
					getProvinsi(data.air.id_provinsi)
					getKabupaten(data.air.id_provinsi,data.air.id_kabupaten)
					getKecamatan(data.air.id_kabupaten,data.air.id_kecamatan)
					getKelurahan(data.air.id_kecamatan,data.air.id_kelurahan)
				}
				else
				{
					getProvinsi(data.air.id_provinsi)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>geografi_sumberAir/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
