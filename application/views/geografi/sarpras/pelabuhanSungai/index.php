		...
		<div class="col-md-12 col-lg-12">
			<div class="card" style="overflow:auto;">
				<div class="card-header">
					<div class="card-title">Pelabuhan Sungai</div>
				...
				<div class="card-body">
					<div class="row">
                        <!-- <div class="col-md-12 col-lg-12 card-title">Data Pantai</div> -->
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataPelabuhanSungai as $sungai): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('GEO','update')): ?>
										<button onclick="editModal(`<?= encrypt($sungai->id_pelabuhan_sungai); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($sungai->id_pelabuhan_sungai); ?>`,'<?= $sungai->nama_pelabuhan ?>')"
									...
									<td><?= $no++ ?></td>
									<td><?= $sungai->nama_satker ?></td>
									<td><?= $sungai->wilayah ?></td>
									<td><?= $sungai->nama_pelabuhan ?></td>
									<td><?= $sungai->nama_sungai ?></td>
									<td><?= $sungai->jarak_dari_laut ?></td>
									<td><?= $sungai->pasang_tinggi ?></td>
									<td><?= $sungai->surut_rendah ?></td>
									<td><?= $sungai->draft_maks ?></td>
									<td><?= $sungai->lebar_kapal_maks ?></td>
									<td><?= $sungai->keterangan ?></td>
									<td><?= $sungai->nama_pegawai ?></td>
									<td><?= $sungai->LastUpdated ?></td>
								</tr>
								<?php endforeach ?>
							</tbody>
						</table>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="river">Nama Sungai</label>
								<div class="col-md-9">
									<input type="text" id="" name="river" class="form-control">
									<div class="invalid-feedback warning-river"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="range">Jarak Dari Laut (Km)</label>
								<div class="col-md-9">
									<input type="text" onkeypress="return isNumberKey(event)" id="" name="range" class="form-control">
									<div class="invalid-feedback warning-range"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="waves">Pasang Tinggi (m)</label>
								<div class="col-md-9">
									<input type="text" onkeypress="return isNumberKey(event)" id="" name="waves" class="form-control">
									<div class="invalid-feedback warning-waves"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="recede">Surut Rendah (m)</label>
								<div class="col-md-9">
									<input type="text" onkeypress="return isNumberKey(event)" id="" name="recede" class="form-control">
									<div class="invalid-feedback warning-recede"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="draft">Draft Maks (m)</label>
								<div class="col-md-9">
									<input type="text" onkeypress="return isNumberKey(event)" id="" name="draft" class="form-control">
									<div class="invalid-feedback warning-draft"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="shipWidth">Lebar Kapal Maks (m)</label>
								<div class="col-md-9">
									<input type="text" onkeypress="return isNumberKey(event)" id="" name="shipWidth" class="form-control">
									<div class="invalid-feedback warning-shipWidth"></div>
								</div>
							</div>
							<div class="form-group row">
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/geografi_pelabuhanSungai/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>geografi_pelabuhanSungais/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>geografi_pelabuhanSungai/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('input[name="satker"]').val(data.pelabuhanSungai.id_satker);
				$('input[name="harbor"]').val(data.pelabuhanSungai.nama_pelabuhan);
				$('input[name="river"]').val(data.pelabuhanSungai.nama_sungai);
				$('input[name="range"]').val(data.pelabuhanSungai.jarak_dari_laut);
				$('input[name="waves"]').val(data.pelabuhanSungai.pasang_tinggi);
				$('input[name="recede"]').val(data.pelabuhanSungai.surut_rendah);
				$('input[name="draft"]').val(data.pelabuhanSungai.draft_maks);
				$('input[name="shipWidth"]').val(data.pelabuhanSungai.lebar_kapal_maks);
				$('input[name="notes"]').val(data.pelabuhanSungai.keterangan);
				$('input[name="flag_locationedit"]').val(data.pelabuhanSungai.flag_location);
				//$("select[name=satker] option[value="+data.pelabuhanSungai.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.pelabuhanSungai.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.pelabuhanSungai.id_provinsi+"]").attr('selected','selected');

				$("#satkerEdit").val(data.pelabuhanSungai.id_satker);
				$("#provinsiEdit").val(data.pelabuhanSungai.id_provinsi);
				...
				// getProvinsi(data.pelabuhanSungai.id_provinsi)
				// getKabupaten(data.pelabuhanSungai.id_provinsi,data.pelabuhanSungai.id_kabupaten)
				// getKecamatan(data.pelabuhanSungai.id_kabupaten,data.pelabuhanSungai.id_kecamatan)
				// getKelurahan(data.pelabuhanSungai.id_kecamatan,data.pelabuhanSungai.id_kelurahan)

				if(data.pelabuhanSungai.flag_location == 'prov')
				{
					getProvinsi(data.pelabuhanSungai.id_provinsi)
					getKabupaten(data.pelabuhanSungai.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.pelabuhanSungai.flag_location == 'kab')
				{
					getProvinsi(data.pelabuhanSungai.id_provinsi)
					getKabupaten(data.pelabuhanSungai.id_provinsi,data.pelabuhanSungai.id_kabupaten)
					getKecamatan(data.pelabuhanSungai.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.pelabuhanSungai.flag_location == 'kec')
				{
					getProvinsi(data.pelabuhanSungai.id_provinsi)
					getKabupaten(data.pelabuhanSungai.id_provinsi,data.pelabuhanSungai.id_kabupaten)
					getKecamatan(data.pelabuhanSungai.id_kabupaten,data.pelabuhanSungai.id_kecamatan)
					getKelurahan(data.pelabuhanSungai.id_kecamatan,0)
				}
				else if(data.pelabuhanSungai.flag_location == 'kel')
				{
					getProvinsi(data.pelabuhanSungai.id_provinsi)
					getKabupaten(data.pelabuhanSungai.id_provinsi,data.pelabuhanSungai.id_kabupaten)
					getKecamatan(data.pelabuhanSungai.id_kabupaten,data.pelabuhanSungai.id_kecamatan)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>geografi_pelabuhanSungai/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
