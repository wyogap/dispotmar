		...
		<div class="col-md-12 col-lg-12">
			<div class="card" style="overflow:auto;">
				<div class="card-header">
					<div class="card-title">Pelabuhan Perikanan</div>
				...
				<div class="card-body">
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataPelabuhanIkan as $ikan): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('GEO','update')): ?>
										<button onclick="editModal(`<?= encrypt($ikan->id_pelabuhan_perikanan); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($ikan->id_pelabuhan_perikanan); ?>`,'<?= $ikan->nama_pelabuhan ?>')"
									...
									<td><?= $ikan->wilayah ?></td>
									<td><?= $ikan->nama_pelabuhan ?></td>
									<td><?= $ikan->kelas_pelabuhanikan ?></td>
									<td><?= $ikan->wpp ?></td>
									<td><?= $ikan->status ?></td>
									<td><?= $ikan->pengelola ?></td>
									<td><?= $ikan->fasilitas ?></td>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="harbor">Nama Pelabuhan</label>
								<div class="col-md-9">
									<input type="text" id="" name="harbor" class="form-control">
									<div class="invalid-feedback warning-harbor"></div>
								</div>
							</div>
							<!-- <div class="form-group row">
								<label class="col-md-3 col-form-label" for="class">Kelas Pelabuhan</label>
								<div class="col-md-9">
									<input type="text" id="" name="class" class="form-control">
									<div class="invalid-feedback warning-class"></div>
								</div>
							</div> -->
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="class">Kelas Pelabuhan</label>
								<div class="col-md-9">
									<select class="form-control" name="class" id="class" style="width: 100%;">
										<?php foreach($kelasPelabuhan as $kelas): ?>
											<option value="<?= $kelas->id_kelas_pelabuhanikan ?>"><?= $kelas->nama ?></option>
										<?php endforeach?>
									</select>
									<div class="invalid-feedback warning-class"></div>
								</div>
							...
							<!-- <div class="form-group row">
								<label class="col-md-3 col-form-label" for="wpp">WPP</label>
								<div class="col-md-9">
									<input type="text" id="" name="wpp" class="form-control">
									<div class="invalid-feedback warning-wpp"></div>
								</div>
							</div> -->
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="wpp">WPP</label>
								<div class="col-md-9">
									<select class="form-control" name="wpp" id="wpp" style="width: 100%;">
										<?php foreach($wpps as $wpp): ?>
											<option value="<?= $wpp->id_wpp ?>"><?= $wpp->nama ?></option>
										<?php endforeach?>
									</select>
									<div class="invalid-feedback warning-wpp"></div>
								</div>
							...
							<div class="form-group row">
								...
								<div class="col-md-9">
									...
									<div class="invalid-feedback warning-status"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="manager">Pengelola</label>
								<div class="col-md-9">
									<input type="text" id="" name="manager" class="form-control">
									<div class="invalid-feedback warning-manager"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="facility">Fasilitas</label>
								<div class="col-md-9">
									<input type="text" id="" name="facility" class="form-control">
									<div class="invalid-feedback warning-facility"></div>
								</div>
							</div>
							<div class="form-group row">
			...
			<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="material">Kelas Pelabuhan</label>
								<div class="col-md-9">
									<select class="form-control" name="class" id="classEdit" style="width: 100%;">
										<?php foreach($kelasPelabuhan as $kelas): ?>
											<option value="<?= $kelas->id_kelas_pelabuhanikan ?>"><?= $kelas->nama ?></option>
										<?php endforeach?>
									</select>
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="material">WPP</label>
								<div class="col-md-9">
									<select class="form-control" name="wpp" id="wppEdit" style="width: 100%;">
										<?php foreach($wpps as $wpp): ?>
											<option value="<?= $wpp->id_wpp ?>"><?= $wpp->nama ?></option>
										<?php endforeach?>
									</select>
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/geografi_pelabuhanIkan/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>geografi_pelabuhanIkans/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>geografi_pelabuhanIkan/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('select[name=class]').find('option:selected').removeAttr('selected');
				$('select[name=wpp]').find('option:selected').removeAttr('selected');
				$('input[name="id"]').val(id);
				$('input[name="satker"]').val(data.pelabuhanIkan.id_satker);
				$('input[name="harbor"]').val(data.pelabuhanIkan.nama_pelabuhan);
				$('input[name="status"]').val(data.pelabuhanIkan.status);
				$('input[name="manager"]').val(data.pelabuhanIkan.pengelola);
				$('input[name="facility"]').val(data.pelabuhanIkan.fasilitas);
				$('input[name="notes"]').val(data.pelabuhanIkan.keterangan);
				$('input[name="flag_locationedit"]').val(data.pelabuhanIkan.flag_location);
				//$("select[name=satker] option[value="+data.pelabuhanIkan.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.pelabuhanIkan.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.pelabuhanIkan.id_provinsi+"]").attr('selected','selected');
				//$("select[name=class] option[value="+data.pelabuhanIkan.id_kelas_pelabuhanikan+"]").attr('selected','selected');
				//$("select[name=wpp] option[value="+data.pelabuhanIkan.id_wpp+"]").attr('selected','selected');

				$("#satkerEdit").val(data.pelabuhanIkan.id_satker);
				$("#provinsiEdit").val(data.pelabuhanIkan.id_provinsi);
				$("#classEdit").val(data.pelabuhanIkan.id_kelas_pelabuhanikan);
				$("#wppEdit").val(data.pelabuhanIkan.id_wpp);
				...
				$("#classEdit").trigger('change');
				$("#wppEdit").trigger('change');

				// getProvinsi(data.pelabuhanIkan.id_provinsi)
				// getKabupaten(data.pelabuhanIkan.id_provinsi,data.pelabuhanIkan.id_kabupaten)
				// getKecamatan(data.pelabuhanIkan.id_kabupaten,data.pelabuhanIkan.id_kecamatan)
				// getKelurahan(data.pelabuhanIkan.id_kecamatan,data.pelabuhanIkan.id_kelurahan)

				if(data.pelabuhanIkan.flag_location == 'prov')
				{
					getProvinsi(data.pelabuhanIkan.id_provinsi)
					getKabupaten(data.pelabuhanIkan.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.pelabuhanIkan.flag_location == 'kab')
				{
					getProvinsi(data.pelabuhanIkan.id_provinsi)
					getKabupaten(data.pelabuhanIkan.id_provinsi,data.pelabuhanIkan.id_kabupaten)
					getKecamatan(data.pelabuhanIkan.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.pelabuhanIkan.flag_location == 'kec')
				{
					getProvinsi(data.pelabuhanIkan.id_provinsi)
					getKabupaten(data.pelabuhanIkan.id_provinsi,data.pelabuhanIkan.id_kabupaten)
					getKecamatan(data.pelabuhanIkan.id_kabupaten,data.pelabuhanIkan.id_kecamatan)
					getKelurahan(data.pelabuhanIkan.id_kecamatan,0)
				}
				else if(data.pelabuhanIkan.flag_location == 'kel')
				{
					getProvinsi(data.pelabuhanIkan.id_provinsi)
					getKabupaten(data.pelabuhanIkan.id_provinsi,data.pelabuhanIkan.id_kabupaten)
					getKecamatan(data.pelabuhanIkan.id_kabupaten,data.pelabuhanIkan.id_kecamatan)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>geografi_pelabuhanIkan/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
