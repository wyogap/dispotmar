<div class="section">
	...
	<div class="page-header">
		...
		<li class="breadcrumb-item"><a href="#"><i class="ti-package mr-1"></i> Geografi</a></li>
			...
			<li class="breadcrumb-item active" aria-current="page">Industri Perikanan</li>
		</ol>
	</div>
	<!-- Page-header closed -->
		...
		<div class="col-md-12 col-lg-12">
			<div class="card" style="overflow:auto;">
				<div class="card-header">
					<div class="card-title">Industri Perikanan</div>
				...
				<div class="card-body">
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataIkan as $ikan): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('KONSOS','update')): ?>
										<button onclick="editModal(`<?= encrypt($ikan->id_industri_perikanan); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($ikan->id_industri_perikanan); ?>`,'<?= $ikan->nama_perusahaan ?>')"
									...
									<td><?= $no++ ?></td>
									<td><?= $ikan->nama_satker ?></td>
									<td><?= $ikan->wilayah ?></td>
									<td><?= $ikan->nama_perusahaan ?></td>
									<td><?= $ikan->gt_kapal ?></td>
									<td><?= $ikan->jumlah_kapal ?></td>
									<td><?= $ikan->namaikan ?></td>
									<td><?= $ikan->alamat ?></td>
									<td><?= $ikan->pemilik ?></td>
									<td><?= $ikan->data_pemilik ?></td>
									<td><?= $ikan->hasil_produksi ?></td>
									<td><?= $ikan->pemanfaatan ?></td>
									<td><?= $ikan->omzet ?></td>
									<td><?= $ikan->keterangan ?></td>
									<td><?= $ikan->nama_pegawai ?></td>
									<td><?= $ikan->LastUpdated ?></td>
								</tr>
								<?php endforeach ?>
							</tbody>
						</table>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="nama">Nama Perusahaan</label>
								<div class="col-md-9">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="gtKapal">GT Kapal</label>
								<div class="col-md-9">
									<input type="text" onkeypress="return isNumberKey(event)" id="" name="gtKapal" class="form-control">
									<div class="invalid-feedback warning-gtKapal"></div>
								</div>
							</div>
							<div class="form-group row">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="material">Jenis Ikan</label>
								<div class="col-md-9">
									<select class="form-control" name="type" id="type" style="width: 100%;">
										<?php foreach($jenisIkan as $jenis): ?>
											<option value="<?= $jenis->id_jenis_ikan ?>"><?= $jenis->nama ?></option>
										<?php endforeach?>
									</select>
							...
							<!-- <div class="form-group row">
								<label class="col-md-3 col-form-label" for="fasilitas">Fasilitas</label>
								<div class="col-md-9">
									<input type="text" id="" name="fasilitas" class="form-control">
									<div class="invalid-feedback warning-fasilitas"></div>
								</div>
							</div> -->
							<div class="form-group row">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="pemilik">Pemilik</label>
								<div class="col-md-9">
									<input type="text" id="" name="pemilik" class="form-control">
									<div class="invalid-feedback warning-pemilik"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="data_pemilik">Data Pemilik</label>
								<div class="col-md-9">
									<input type="text" id="" name="data_pemilik" class="form-control">
									<div class="invalid-feedback warning-data_pemilik"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="hasil_produksi">Hasil Produksi (Ton)</label>
								<div class="col-md-9">
									<input type="text" onkeypress="return isNumberKey(event)" id="" name="hasil_produksi" class="form-control">
									<div class="invalid-feedback warning-hasil_produksi"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="pemanfaatan">Pemanfaatan</label>
								<div class="col-md-9">
									<input type="text" id="" name="pemanfaatan" class="form-control">
									<div class="invalid-feedback warning-pemanfaatan"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="omzet">Omzet (Rp)</label>
								<div class="col-md-9">
									<input type="text" onkeypress="return isNumberKey(event)" id="" name="omzet" class="form-control">
									<div class="invalid-feedback warning-omzet"></div>
								</div>
							</div>
							<div class="form-group row">
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/geografi_industriIkan/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>geografi_industriIkans/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>geografi_industriIkan/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('input[name="satker"]').val(data.ikan.id_satker);
				$('input[name="nama"]').val(data.ikan.nama_perusahaan);
				$('input[name="gtKapal"]').val(data.ikan.gt_kapal);
				$('input[name="jumlah_kapal"]').val(data.ikan.jumlah_kapal);
				// $('input[name="fasilitas"]').val(data.ikan.fasilitas);
				$('input[name="alamat"]').val(data.ikan.alamat);
				$('input[name="pemilik"]').val(data.ikan.pemilik);
				$('input[name="data_pemilik"]').val(data.ikan.data_pemilik);
				$('input[name="hasil_produksi"]').val(data.ikan.hasil_produksi);
				$('input[name="pemanfaatan"]').val(data.ikan.pemanfaatan);
				$('input[name="omzet"]').val(data.ikan.omzet);
				$('input[name="notes"]').val(data.ikan.keterangan);
				$('input[name="flag_locationedit"]').val(data.ikan.flag_location);
				//$("select[name=satker] option[value="+data.ikan.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.ikan.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.ikan.id_provinsi+"]").attr('selected','selected');

				$("#satkerEdit").val(data.ikan.id_satker);
				$("#provinsiEdit").val(data.ikan.id_provinsi);
				$("#typeEdit").val(data.ikan.id_jenis_ikan);
				...
				// getProvinsi(data.ikan.id_provinsi)
				// getKabupaten(data.ikan.id_provinsi,data.ikan.id_kabupaten)
				// getKecamatan(data.ikan.id_kabupaten,data.ikan.id_kecamatan)
				// getKelurahan(data.ikan.id_kecamatan,data.ikan.id_kelurahan)

				if(data.ikan.flag_location == 'prov')
				{
					getProvinsi(data.ikan.id_provinsi)
					getKabupaten(data.ikan.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.ikan.flag_location == 'kab')
				{
					getProvinsi(data.ikan.id_provinsi)
					getKabupaten(data.ikan.id_provinsi,data.ikan.id_kabupaten)
					getKecamatan(data.ikan.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.ikan.flag_location == 'kec')
				{
					getProvinsi(data.ikan.id_provinsi)
					getKabupaten(data.ikan.id_provinsi,data.ikan.id_kabupaten)
					getKecamatan(data.ikan.id_kabupaten,data.ikan.id_kecamatan)
					getKelurahan(data.ikan.id_kecamatan,0)
				}
				else if(data.ikan.flag_location == 'kel')
				{
					getProvinsi(data.ikan.id_provinsi)
					getKabupaten(data.ikan.id_provinsi,data.ikan.id_kabupaten)
					getKecamatan(data.ikan.id_kabupaten,data.ikan.id_kecamatan)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>geografi_industriIkan/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
