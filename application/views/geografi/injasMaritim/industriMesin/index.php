<div class="section">
	...
	<div class="page-header">
		...
		<li class="breadcrumb-item"><a href="#"><i class="ti-package mr-1"></i> Geografi</a></li>
			...
			<li class="breadcrumb-item active" aria-current="page">Industri Mesin dan Spare Part Perkapalan</li>
		</ol>
	</div>
	<!-- Page-header closed -->
		...
		<div class="col-md-12 col-lg-12">
			<div class="card" style="overflow:auto;">
				<div class="card-header">
					<div class="card-title">Industri Mesin dan Spare Part Perkapalan</div>
				...
				<div class="card-body">
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataMesin as $mesin): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('GEO','update')): ?>
										<button onclick="editModal(`<?= encrypt($mesin->id_industri_mesin); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($mesin->id_industri_mesin); ?>`,'<?= $mesin->nama_perusahaan ?>')"
									...
									<td><?= $no++ ?></td>
									<td><?= $mesin->nama_satker ?></td>
									<td><?= $mesin->wilayah ?></td>
									<td><?= $mesin->nama_perusahaan ?></td>
									<td><?= $mesin->hasil_produksi ?></td>
									<td><?= $mesin->besaran_produksi ?></td>
									<td><?= $mesin->status_kepemilikan ?></td>
									<td><?= $mesin->penggunaan_hasil_produksi ?></td>
									<td><?= $mesin->keterangan ?></td>
									<td><?= $mesin->nama_pegawai ?></td>
									<td><?= $mesin->LastUpdated ?></td>
								</tr>
								<?php endforeach ?>
							</tbody>
						</table>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="name">Nama Perusahaan</label>
								<div class="col-md-9">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="results">Hasil Produksi</label>
								<div class="col-md-9">
									<input type="text" id="" name="results" class="form-control">
									<div class="invalid-feedback warning-results"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="production">Besaran Produksi / Bulan</label>
								<div class="col-md-9">
									<input type="number" id="" name="production" class="form-control">
									<div class="invalid-feedback warning-production"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="status">Status Kepemilikan</label>
								<div class="col-md-9">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="use">Penggunaan Hasil Produksi</label>
								<div class="col-md-9">
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/geografi_industriMesin/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>geografi_industriMesins/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>geografi_industriMesin/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('input[name="satker"]').val(data.mesin.id_satker);
				$('input[name="name"]').val(data.mesin.nama_perusahaan);
				$('input[name="results"]').val(data.mesin.hasil_produksi);
				$('input[name="production"]').val(data.mesin.besaran_produksi);
				$('input[name="status"]').val(data.mesin.status_kepemilikan);
				$('input[name="use"]').val(data.mesin.penggunaan_hasil_produksi);
				$('input[name="notes"]').val(data.mesin.keterangan);
				$('input[name="flag_locationedit"]').val(data.mesin.flag_location);
				//$("select[name=satker] option[value="+data.mesin.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.mesin.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.mesin.id_provinsi+"]").attr('selected','selected');

				$("#satkerEdit").val(data.mesin.id_satker);
				$("#provinsiEdit").val(data.mesin.id_provinsi);
				...
				// getProvinsi(data.mesin.id_provinsi)
				// getKabupaten(data.mesin.id_provinsi,data.mesin.id_kabupaten)
				// getKecamatan(data.mesin.id_kabupaten,data.mesin.id_kecamatan)
				// getKelurahan(data.mesin.id_kecamatan,data.mesin.id_kelurahan)

				if(data.mesin.flag_location == 'prov')
				{
					getProvinsi(data.mesin.id_provinsi)
					getKabupaten(data.mesin.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.mesin.flag_location == 'kab')
				{
					getProvinsi(data.mesin.id_provinsi)
					getKabupaten(data.mesin.id_provinsi,data.mesin.id_kabupaten)
					getKecamatan(data.mesin.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.mesin.flag_location == 'kec')
				{
					getProvinsi(data.mesin.id_provinsi)
					getKabupaten(data.mesin.id_provinsi,data.mesin.id_kabupaten)
					getKecamatan(data.mesin.id_kabupaten,data.mesin.id_kecamatan)
					getKelurahan(data.mesin.id_kecamatan,0)
				}
				else if(data.mesin.flag_location == 'kel')
				{
					getProvinsi(data.mesin.id_provinsi)
					getKabupaten(data.mesin.id_provinsi,data.mesin.id_kabupaten)
					getKecamatan(data.mesin.id_kabupaten,data.mesin.id_kecamatan)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>geografi_industriMesin/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
