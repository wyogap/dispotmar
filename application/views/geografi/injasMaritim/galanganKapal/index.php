<div class="section">
	...
	<div class="page-header">
		...
		<li class="breadcrumb-item"><a href="#"><i class="ti-package mr-1"></i> Geografi</a></li>
			...
			<li class="breadcrumb-item active" aria-current="page">Galangan Kapal</li>
		</ol>
	</div>
	<!-- Page-header closed -->
		...
		<div class="col-md-12 col-lg-12">
			<div class="card" style="overflow:auto;">
				<div class="card-header">
					<div class="card-title">Galangan Kapal</div>
				...
				<div class="card-body">
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataGalanganKapal as $kapal): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('GEO','update')): ?>
										<button onclick="editModal(`<?= encrypt($kapal->id_galangan_kapal); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($kapal->id_galangan_kapal); ?>`,'<?= $kapal->nama_galangan ?>')"
									...
									<td><?= $kapal->wilayah ?></td>
									<td><?= $kapal->nama_galangan ?></td>
									<td><?= $kapal->pemilik ?></td>
									<td><?= $kapal->maks_gt ?></td>
									<td><?= $kapal->status_kepemilikan ?></td>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="name">Nama Galangan</label>
								<div class="col-md-9">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="maksGT">Maks GT</label>
								<div class="col-md-9">
									<input type="text" onkeypress="return isNumberKey(event)" id="" name="maksGT" class="form-control">
									<div class="invalid-feedback warning-maksGT"></div>
								</div>
							</div>
							<div class="form-group row">
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/geografi_galanganKapal/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>geografi_galanganKapals/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>geografi_galanganKapal/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('input[name="satker"]').val(data.galanganKapal.id_satker);
				$('input[name="name"]').val(data.galanganKapal.nama_galangan);
				$('input[name="owner"]').val(data.galanganKapal.pemilik);
				$('input[name="maksGT"]').val(data.galanganKapal.maks_gt);
				$('input[name="status"]').val(data.galanganKapal.status_kepemilikan);
				$('input[name="facility"]').val(data.galanganKapal.fasilitas);
				$('input[name="notes"]').val(data.galanganKapal.keterangan);
				$('input[name="flag_locationedit"]').val(data.galanganKapal.flag_location);
				//$("select[name=satker] option[value="+data.galanganKapal.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.galanganKapal.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.galanganKapal.id_provinsi+"]").attr('selected','selected');

				$("#satkerEdit").val(data.galanganKapal.id_satker);
				$("#provinsiEdit").val(data.galanganKapal.id_provinsi);
				...
				// getProvinsi(data.galanganKapal.id_provinsi)
				// getKabupaten(data.galanganKapal.id_provinsi,data.galanganKapal.id_kabupaten)
				// getKecamatan(data.galanganKapal.id_kabupaten,data.galanganKapal.id_kecamatan)
				// getKelurahan(data.galanganKapal.id_kecamatan,data.galanganKapal.id_kelurahan)

				if(data.galanganKapal.flag_location == 'prov')
				{
					getProvinsi(data.galanganKapal.id_provinsi)
					getKabupaten(data.galanganKapal.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.galanganKapal.flag_location == 'kab')
				{
					getProvinsi(data.galanganKapal.id_provinsi)
					getKabupaten(data.galanganKapal.id_provinsi,data.galanganKapal.id_kabupaten)
					getKecamatan(data.galanganKapal.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.galanganKapal.flag_location == 'kec')
				{
					getProvinsi(data.galanganKapal.id_provinsi)
					getKabupaten(data.galanganKapal.id_provinsi,data.galanganKapal.id_kabupaten)
					getKecamatan(data.galanganKapal.id_kabupaten,data.galanganKapal.id_kecamatan)
					getKelurahan(data.galanganKapal.id_kecamatan,0)
				}
				else if(data.galanganKapal.flag_location == 'kel')
				{
					getProvinsi(data.galanganKapal.id_provinsi)
					getKabupaten(data.galanganKapal.id_provinsi,data.galanganKapal.id_kabupaten)
					getKecamatan(data.galanganKapal.id_kabupaten,data.galanganKapal.id_kecamatan)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>geografi_galanganKapal/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
