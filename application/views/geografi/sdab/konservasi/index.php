<div class="section">
	...
	<div class="page-header">
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active" aria-current="page">Konservasi</li>
		</ol>
	</div>
	<!-- Page-header closed -->
		...
		<div class="col-md-12 col-lg-12">
			<div class="card" style="overflow:auto;">
				<div class="card-header">
					<div class="card-title">Konservasi Lingkungan Hidup</div>
				...
				<div class="card-body">
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataKonservasi as $konservasi): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('GEO','update')): ?>
										<button onclick="editModal(`<?= encrypt($konservasi->id_konservasi); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($konservasi->id_konservasi); ?>`,'Konservasi')"
									...
									<td><?= $no++ ?></td>
									<td><?= $konservasi->nama_satker ?></td>
									<td><?= $konservasi->wilayah ?></td>
									<td><?= $konservasi->jenis_konservasi ?></td>
									<td><?= $konservasi->penanggung_jawab ?></td>
									<td><?= $konservasi->luas ?></td>
									<td><?= $konservasi->keterangan ?></td>
									<td><?= $konservasi->nama_pegawai ?></td>
									<td><?= $konservasi->LastUpdated ?></td>
								</tr>
								<?php endforeach ?>
							</tbody>
						</table>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<!-- <div class="form-group row">
								<label class="col-md-3 col-form-label" for="type">Jenis yang Dikonservasikan</label>
								<div class="col-md-9">
							...
							<div class="form-group row">
								...
								<div class="col-md-9">
									<select class="form-control" name="type" id="type" style="width: 100%;">
										<?php foreach($jenisKonservasi as $jenis): ?>
											<option value="<?= $jenis->id_jenis_konservasi ?>"><?= $jenis->nama ?></option>
										<?php endforeach?>
									</select>
									<div class="invalid-feedback warning-type"></div>
								</div>
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="responsible">Penanggung Jawab</label>
								<div class="col-md-9">
									<input type="text" id="" name="responsible" class="form-control">
									<div class="invalid-feedback warning-responsible"></div>
								</div>
							</div>
							<div class="form-group row">
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="material">Jenis yang Dikonservasikan</label>
								<div class="col-md-9">
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/geografi_konservasi/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>geografi_konservasis/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>geografi_konservasi/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('input[name="satker"]').val(data.konservasi.id_satker);
				$('input[name="responsible"]').val(data.konservasi.penanggung_jawab);
				$('input[name="area"]').val(data.konservasi.luas);
				$('input[name="notes"]').val(data.konservasi.keterangan);
				$('input[name="flag_locationedit"]').val(data.konservasi.flag_location);
				//$("select[name=satker] option[value="+data.konservasi.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.konservasi.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.konservasi.id_provinsi+"]").attr('selected','selected');
				//$("select[name=type] option[value="+data.konservasi.id_jenis_konservasi+"]").attr('selected','selected');

				$("#satkerEdit").val(data.konservasi.id_satker);
				$("#provinsiEdit").val(data.konservasi.id_provinsi);
				$("#typeEdit").val(data.konservasi.id_jenis_konservasi);
				...
				// getProvinsi(data.konservasi.id_provinsi)
				// getKabupaten(data.konservasi.id_provinsi,data.konservasi.id_kabupaten)
				// getKecamatan(data.konservasi.id_kabupaten,data.konservasi.id_kecamatan)
				// getKelurahan(data.konservasi.id_kecamatan,data.konservasi.id_kelurahan)

				if(data.konservasi.flag_location == 'prov')
				{
					getProvinsi(data.konservasi.id_provinsi)
					getKabupaten(data.konservasi.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.konservasi.flag_location == 'kab')
				{
					getProvinsi(data.konservasi.id_provinsi)
					getKabupaten(data.konservasi.id_provinsi,data.konservasi.id_kabupaten)
					getKecamatan(data.konservasi.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.konservasi.flag_location == 'kec')
				{
					getProvinsi(data.konservasi.id_provinsi)
					getKabupaten(data.konservasi.id_provinsi,data.konservasi.id_kabupaten)
					getKecamatan(data.konservasi.id_kabupaten,data.konservasi.id_kecamatan)
					getKelurahan(data.konservasi.id_kecamatan,0)
				}
				else if(data.konservasi.flag_location == 'kel')
				{
					getProvinsi(data.konservasi.id_provinsi)
					getKabupaten(data.konservasi.id_provinsi,data.konservasi.id_kabupaten)
					getKecamatan(data.konservasi.id_kabupaten,data.konservasi.id_kecamatan)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>geografi_konservasi/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
