		...
		<div class="col-md-12 col-lg-12">
			<div class="card" style="overflow:auto;">
				<div class="card-header">
					<div class="card-title">Perkebunan</div>
				...
				<div class="card-body">
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataPerkebunan as $perkebunan): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('GEO','update')): ?>
										<button onclick="editModal(`<?= encrypt($perkebunan->id_perkebunan); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($perkebunan->id_perkebunan); ?>`,'Perkebunan')"
									...
									<td><?= $no++ ?></td>
									<td><?= $perkebunan->nama_satker ?></td>
									<td><?= $perkebunan->wilayah ?></td>
									<td><?= $perkebunan->nama_komoditas ?></td>
									<td><?= $perkebunan->luas ?></td>
									<td><?= $perkebunan->tonase_hasil ?></td>
									<td><?= $perkebunan->masa_panen ?></td>
									<td><?= $perkebunan->keterangan ?></td>
									<td><?= $perkebunan->nama_pegawai ?></td>
									<td><?= $perkebunan->LastUpdated ?></td>
								</tr>
								<?php endforeach ?>
							</tbody>
						</table>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								...
								<div class="col-md-9">
									...
									<div class="col-md-15">
										...
										<input type="text" id="flag_location" name="flag_location" style="display:block;" class="form-control">
									</div>
								</div>
							</div>
							<!-- <div class="form-group row">
								<label class="col-md-3 col-form-label" for="type">Jenis Tanaman</label>
								<div class="col-md-9">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="area">Luas (Ha)</label>
								<div class="col-md-9">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="result">Tonase Hasil</label>
								<div class="col-md-9">
							...
							<div class="form-group row">
								...
								<div class="col-md-9">
									<input type="number" id="" name="period" class="form-control">
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/geografi_perkebunan/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>geografi_perkebunans/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>geografi_perkebunan/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('input[name="satker"]').val(data.perkebunan.id_satker);
				$('input[name="area"]').val(data.perkebunan.luas);
				$('input[name="result"]').val(data.perkebunan.tonase_hasil);
				$('input[name="period"]').val(data.perkebunan.masa_panen);
				$('input[name="notes"]').val(data.perkebunan.keterangan);
				$('input[name="flag_locationedit"]').val(data.perkebunan.flag_location);
				//$("select[name=satker] option[value="+data.perkebunan.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.perkebunan.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.perkebunan.id_provinsi+"]").attr('selected','selected');
				//$("select[name=type] option[value="+data.perkebunan.id_komoditas+"]").attr('selected','selected');

				$("#satkerEdit").val(data.perkebunan.id_satker);
				$("#provinsiEdit").val(data.perkebunan.id_provinsi);
				$("#typeEdit").val(data.perkebunan.id_komoditas);
				...
				// getProvinsi(data.perkebunan.id_provinsi)
				// getKabupaten(data.perkebunan.id_provinsi,data.perkebunan.id_kabupaten)
				// getKecamatan(data.perkebunan.id_kabupaten,data.perkebunan.id_kecamatan)
				// getKelurahan(data.perkebunan.id_kecamatan,data.perkebunan.id_kelurahan)

				if(data.perkebunan.flag_location == 'prov')
				{
					getProvinsi(data.perkebunan.id_provinsi)
					getKabupaten(data.perkebunan.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.perkebunan.flag_location == 'kab')
				{
					getProvinsi(data.perkebunan.id_provinsi)
					getKabupaten(data.perkebunan.id_provinsi,data.perkebunan.id_kabupaten)
					getKecamatan(data.perkebunan.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.perkebunan.flag_location == 'kec')
				{
					getProvinsi(data.perkebunan.id_provinsi)
					getKabupaten(data.perkebunan.id_provinsi,data.perkebunan.id_kabupaten)
					getKecamatan(data.perkebunan.id_kabupaten,data.perkebunan.id_kecamatan)
					getKelurahan(data.perkebunan.id_kecamatan,0)
				}
				else if(data.perkebunan.flag_location == 'kel')
				{
					getProvinsi(data.perkebunan.id_provinsi)
					getKabupaten(data.perkebunan.id_provinsi,data.perkebunan.id_kabupaten)
					getKecamatan(data.perkebunan.id_kabupaten,data.perkebunan.id_kecamatan)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>geografi_perkebunan/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
