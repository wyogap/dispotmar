			...
			<div class="card" style="overflow:auto;">
				...
				<div class="card-body">
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataPertanian as $pertanian): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('GEO','update')): ?>
										<button onclick="editModal(`<?= encrypt($pertanian->id_pertanian); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($pertanian->id_pertanian); ?>`,'Pertanian')"
									...
									<td><?= $no++ ?></td>
									<td><?= $pertanian->nama_satker ?></td>
									<td><?= $pertanian->wilayah ?></td>
									<td><?= $pertanian->nama_komoditas ?></td>
									<td><?= $pertanian->luas ?></td>
									<td><?= $pertanian->tonase_hasil ?></td>
									<td><?= $pertanian->masa_panen ?></td>
									<td><?= $pertanian->keterangan ?></td>
									<td><?= $pertanian->nama_pegawai ?></td>
									<td><?= $pertanian->LastUpdated ?></td>
								</tr>
								<?php endforeach ?>
							</tbody>
						</table>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								...
								<div class="col-md-9">
									<select class="form-control" name="type" id="type" style="width: 100%;">
										<?php foreach($JenisTanaman_pertanian as $jenis): ?>
											<option value="<?= $jenis->id_jenis_tanaman ?>"><?= $jenis->nama ?></option>
										<?php endforeach?>
									</select>
									<div class="invalid-feedback warning-type"></div>
								</div>
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="area">Luas</label>
								<div class="col-md-9">
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/geografi_pertanian/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>geografi_pertanians/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>geografi_pertanian/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('input[name="satker"]').val(data.pertanian.id_satker);
				$('input[name="area"]').val(data.pertanian.luas);
				$('input[name="result"]').val(data.pertanian.tonase_hasil);
				$('input[name="period"]').val(data.pertanian.masa_panen);
				$('input[name="notes"]').val(data.pertanian.keterangan);
				$('input[name="flag_locationedit"]').val(data.pertanian.flag_location);
				//$("select[name=satker] option[value="+data.pertanian.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.pertanian.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.pertanian.id_provinsi+"]").attr('selected','selected');
				//$("select[name=type] option[value="+data.pertanian.id_komoditas+"]").attr('selected','selected');
				//$("select[name=type] option[value="+data.pertanian.id_komoditas+"]").attr('selected','selected');

				$("#satkerEdit").val(data.pertanian.id_satker);
				$("#provinsiEdit").val(data.pertanian.id_provinsi);
				$("#typeEdit").val(data.pertanian.id_komoditas);
				...
				// getProvinsi(data.pertanian.id_provinsi)
				// getKabupaten(data.pertanian.id_provinsi,data.pertanian.id_kabupaten)
				// getKecamatan(data.pertanian.id_kabupaten,data.pertanian.id_kecamatan)
				// getKelurahan(data.pertanian.id_kecamatan,data.pertanian.id_kelurahan)

				if(data.pertanian.flag_location == 'prov')
				{
					getProvinsi(data.pertanian.id_provinsi)
					getKabupaten(data.pertanian.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.pertanian.flag_location == 'kab')
				{
					getProvinsi(data.pertanian.id_provinsi)
					getKabupaten(data.pertanian.id_provinsi,data.pertanian.id_kabupaten)
					getKecamatan(data.pertanian.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.pertanian.flag_location == 'kec')
				{
					getProvinsi(data.pertanian.id_provinsi)
					getKabupaten(data.pertanian.id_provinsi,data.pertanian.id_kabupaten)
					getKecamatan(data.pertanian.id_kabupaten,data.pertanian.id_kecamatan)
					getKelurahan(data.pertanian.id_kecamatan,0)
				}
				else if(data.pertanian.flag_location == 'kel')
				{
					getProvinsi(data.pertanian.id_provinsi)
					getKabupaten(data.pertanian.id_provinsi,data.pertanian.id_kabupaten)
					getKecamatan(data.pertanian.id_kabupaten,data.pertanian.id_kecamatan)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>geografi_pertanian/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
