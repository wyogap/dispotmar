			...
			<div class="card" style="overflow:auto;">
				...
				<div class="card-body">
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataPeternakan as $peternakan): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('GEO','update')): ?>
										<button onclick="editModal(`<?= encrypt($peternakan->id_peternakan); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($peternakan->id_peternakan); ?>`,'Peternakan')"
									...
									<td><?= $no++ ?></td>
									<td><?= $peternakan->nama_satker ?></td>
									<td><?= $peternakan->wilayah ?></td>
									<td><?= $peternakan->nama_komoditas ?></td>
									<td><?= $peternakan->luas ?></td>
									<td><?= $peternakan->tonase_hasil ?></td>
									<td><?= $peternakan->masa_panen ?></td>
									<td><?= $peternakan->keterangan ?></td>
									<td><?= $peternakan->nama_pegawai ?></td>
									<td><?= $peternakan->LastUpdated ?></td>
								</tr>
								<?php endforeach ?>
							</tbody>
						</table>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<!-- <div class="form-group row">
								<label class="col-md-3 col-form-label" for="type">Jenis Hewan</label>
								<div class="col-md-9">
									<select class="form-control" name="type">
										<?php foreach($KomoditasPangan as $jenis): ?>
											<option value="<?= $jenis->id_komoditas ?>"><?= $jenis->nama_komoditas ?></option>
										<?php endforeach?>
									</select>
							...
							<div class="form-group row">
								...
								<div class="col-md-9">
									<select class="form-control" name="type" id="type" style="width: 100%;">
										<?php foreach($JenisHewan as $jenis): ?>
											<option value="<?= $jenis->id_jenis_hewan ?>"><?= $jenis->nama_hewan ?></option>
										<?php endforeach?>
									</select>
									<div class="invalid-feedback warning-type"></div>
								</div>
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="area">Luas Daerah (Ha)</label>
								<div class="col-md-9">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="result">Tonase Hasil (Ton)</label>
								<div class="col-md-9">
									<input type="text" onkeypress="return isNumberKey(event)" id="" name="result" class="form-control">
									<div class="invalid-feedback warning-result"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="period">Masa Panen</label>
								<div class="col-md-9">
									<input type="text" id="" name="period" class="form-control">
									<div class="invalid-feedback warning-period"></div>
								</div>
							</div>
							<div class="form-group row">
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="area">Luas Dearah (Ha)</label>
								<div class="col-md-9">
									<input type="text" onkeypress="return isNumberKey(event)" id="" name="area" class="form-control">
									<div class="invalid-feedback warning-area"></div>
								</div>
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/geografi_peternakan/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>geografi_peternakans/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>geografi_peternakan/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('input[name="satker"]').val(data.peternakan.id_satker);
				$('input[name="area"]').val(data.peternakan.luas);
				$('input[name="result"]').val(data.peternakan.tonase_hasil);
				$('input[name="period"]').val(data.peternakan.masa_panen);
				$('input[name="notes"]').val(data.peternakan.keterangan);
				$('input[name="flag_locationedit"]').val(data.peternakan.flag_location);
				//$("select[name=satker] option[value="+data.peternakan.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.peternakan.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.peternakan.id_provinsi+"]").attr('selected','selected');
				//$("select[name=type] option[value="+data.peternakan.id_komoditas+"]").attr('selected','selected');

				$("#satkerEdit").val(data.peternakan.id_satker);
				$("#provinsiEdit").val(data.peternakan.id_provinsi);
				$("#typeEdit").val(data.peternakan.id_komoditas);
				...
				// getProvinsi(data.peternakan.id_provinsi)
				// getKabupaten(data.peternakan.id_provinsi,data.peternakan.id_kabupaten)
				// getKecamatan(data.peternakan.id_kabupaten,data.peternakan.id_kecamatan)
				// getKelurahan(data.peternakan.id_kecamatan,data.peternakan.id_kelurahan)

				if(data.peternakan.flag_location == 'prov')
				{
					getProvinsi(data.peternakan.id_provinsi)
					getKabupaten(data.peternakan.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.peternakan.flag_location == 'kab')
				{
					getProvinsi(data.peternakan.id_provinsi)
					getKabupaten(data.peternakan.id_provinsi,data.peternakan.id_kabupaten)
					getKecamatan(data.peternakan.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.peternakan.flag_location == 'kec')
				{
					getProvinsi(data.peternakan.id_provinsi)
					getKabupaten(data.peternakan.id_provinsi,data.peternakan.id_kabupaten)
					getKecamatan(data.peternakan.id_kabupaten,data.peternakan.id_kecamatan)
					getKelurahan(data.peternakan.id_kecamatan,0)
				}
				else if(data.peternakan.flag_location == 'kel')
				{
					getProvinsi(data.peternakan.id_provinsi)
					getKabupaten(data.peternakan.id_provinsi,data.peternakan.id_kabupaten)
					getKecamatan(data.peternakan.id_kabupaten,data.peternakan.id_kecamatan)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>geografi_peternakan/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
