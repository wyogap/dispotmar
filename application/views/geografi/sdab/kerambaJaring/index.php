<div class="section">
	...
	<div class="page-header">
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active" aria-current="page">Keramba Jaring</li>
		</ol>
	</div>
	<!-- Page-header closed -->
		...
		<div class="col-md-12 col-lg-12">
			<div class="card" style="overflow:auto;">
				<div class="card-header">
					<div class="card-title">Keramba Jaring Apung</div>
				...
				<div class="card-body">
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataKeramba as $keramba): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('GEO','update')): ?>
										<button onclick="editModal(`<?= encrypt($keramba->id_keramba); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($keramba->id_keramba); ?>`,'Keramba')"
									...
									<td><?= $no++ ?></td>
									<td><?= $keramba->nama_satker ?></td>
									<td><?= $keramba->wilayah ?></td>
									<td><?= $keramba->jenis_ikan ?></td>
									<td><?= $keramba->tonase ?></td>
									<td><?= $keramba->penghasilan ?></td>
									<td><?= $keramba->luas ?></td>
									<td><?= $keramba->keterangan ?></td>
									<td><?= $keramba->nama_pegawai ?></td>
									<td><?= $keramba->LastUpdated ?></td>
								</tr>
								<?php endforeach ?>
							</tbody>
						</table>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<!-- <div class="form-group row">
								<label class="col-md-3 col-form-label" for="type">Jenis Ikan</label>
								<div class="col-md-9">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="tonase">Tonase (Ton)</label>
								<div class="col-md-9">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="results">Penghasilan (Rp)</label>
								<div class="col-md-9">
									<input type="number" id="" name="results" class="form-control">
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/geografi_kerambaJaring/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>geografi_kerambaJarings/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>geografi_kerambaJaring/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('input[name="satker"]').val(data.keramba.id_satker);
				$('input[name="area"]').val(data.keramba.luas);
				$('input[name="results"]').val(data.keramba.penghasilan);
				$('input[name="tonase"]').val(data.keramba.tonase);
				$('input[name="notes"]').val(data.keramba.keterangan);
				$('input[name="flag_locationedit"]').val(data.keramba.flag_location);
				//$("select[name=satker] option[value="+data.keramba.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.keramba.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.keramba.id_provinsi+"]").attr('selected','selected');
				//$("select[name=type] option[value="+data.keramba.id_jenis_ikan+"]").attr('selected','selected');

				$("#satkerEdit").val(data.keramba.id_satker);
				$("#provinsiEdit").val(data.keramba.id_provinsi);
				$("#typeEdit").val(data.keramba.id_jenis_ikan);
				...
				// getProvinsi(data.keramba.id_provinsi)
				// getKabupaten(data.keramba.id_provinsi,data.keramba.id_kabupaten)
				// getKecamatan(data.keramba.id_kabupaten,data.keramba.id_kecamatan)
				// getKelurahan(data.keramba.id_kecamatan,data.keramba.id_kelurahan)

				if(data.keramba.flag_location == 'prov')
				{
					getProvinsi(data.keramba.id_provinsi)
					getKabupaten(data.keramba.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.keramba.flag_location == 'kab')
				{
					getProvinsi(data.keramba.id_provinsi)
					getKabupaten(data.keramba.id_provinsi,data.keramba.id_kabupaten)
					getKecamatan(data.keramba.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.keramba.flag_location == 'kec')
				{
					getProvinsi(data.keramba.id_provinsi)
					getKabupaten(data.keramba.id_provinsi,data.keramba.id_kabupaten)
					getKecamatan(data.keramba.id_kabupaten,data.keramba.id_kecamatan)
					getKelurahan(data.keramba.id_kecamatan,0)
				}
				else if(data.keramba.flag_location == 'kel')
				{
					getProvinsi(data.keramba.id_provinsi)
					getKabupaten(data.keramba.id_provinsi,data.keramba.id_kabupaten)
					getKecamatan(data.keramba.id_kabupaten,data.keramba.id_kecamatan)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>geografi_kerambaJaring/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
