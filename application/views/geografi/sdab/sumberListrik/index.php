<div class="section">
	...
	<div class="page-header">
		...
		<li class="breadcrumb-item"><a href="#"><i class="ti-package mr-1"></i>Geografi</a></li>
			...
			<li class="breadcrumb-item active" aria-current="page">Sumber Listrik</li>
		</ol>
	</div>
	<!-- Page-header closed -->
		...
		<div class="col-md-12 col-lg-12">
			<div class="card" style="overflow:auto;">
				<div class="card-header">
					<div class="card-title">Sumber Listrik</div>
				...
				<div class="card-body">
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataListrik as $listrik): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('GEO','update')): ?>
										<button onclick="editModal(`<?= encrypt($listrik->id_listrik); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($listrik->id_listrik); ?>`,'Listrik')"
									...
									<td><?= $no++ ?></td>
									<td><?= $listrik->nama_satker ?></td>
									<td><?= $listrik->wilayah ?></td>
									<td><?= $listrik->sumber_listrik ?></td>
									<td><?= $listrik->energi_dihasilkan ?></td>
									<td><?= $listrik->luas_cakupan ?></td>
									<td><?= $listrik->keterangan ?></td>
									<td><?= $listrik->nama_pegawai ?></td>
									<td><?= $listrik->LastUpdated ?></td>
								</tr>
								<?php endforeach ?>
							</tbody>
						</table>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<!-- <div class="form-group row">
								<label class="col-md-3 col-form-label" for="sources">Sumber Listrik</label>
								<div class="col-md-9">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="sources">Sumber Listrik</label>
								<div class="col-md-9">
									<select class="form-control" name="sources" id="sources" style="width: 100%;">
										<?php foreach($sumberListrik as $sumber): ?>
											<option value="<?= $sumber->id_sumber_listrik ?>"><?= $sumber->nama ?></option>
										<?php endforeach?>
									</select>
									<div class="invalid-feedback warning-sources"></div>
								</div>
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="results">Energi yang Dihasilkan (Kw)</label>
								<div class="col-md-9">
									<input type="text" onkeypress="return isNumberKey(event)" id="" name="results" class="form-control">
									<div class="invalid-feedback warning-results"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="area">Luas Cakupan</label>
								<div class="col-md-9">
									<input type="text" id="" name="area" class="form-control">
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="material">Sumber Listrik</label>
								<div class="col-md-9">
									<select class="form-control" name="sources" id="sourcesEdit" style="width: 100%;">
										<?php foreach($sumberListrik as $sumber): ?>
											<option value="<?= $sumber->id_sumber_listrik ?>"><?= $sumber->nama ?></option>
										<?php endforeach?>
									</select>
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/geografi_sumberListrik/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>geografi_sumberListriks/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>geografi_sumberListrik/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('select[name=sources]').find('option:selected').removeAttr('selected');
				$('input[name="id"]').val(id);
				$('input[name="satker"]').val(data.listrik.id_satker);
				$('input[name="area"]').val(data.listrik.luas_cakupan);
				$('input[name="results"]').val(data.listrik.energi_dihasilkan);
				$('input[name="notes"]').val(data.listrik.keterangan);
				$('input[name="flag_locationedit"]').val(data.listrik.flag_location);
				//$("select[name=satker] option[value="+data.listrik.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.listrik.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.listrik.id_provinsi+"]").attr('selected','selected');
				//$("select[name=sources] option[value="+data.listrik.id_sumber_listrik+"]").attr('selected','selected');

				$("#satkerEdit").val(data.listrik.id_satker);
				$("#provinsiEdit").val(data.listrik.id_provinsi);
				$("#sourcesEdit").val(data.listrik.id_sumber_listrik);
				...
				$("#sourcesEdit").trigger('change');

				// getProvinsi(data.listrik.id_provinsi)
				// getKabupaten(data.listrik.id_provinsi,data.listrik.id_kabupaten)
				// getKecamatan(data.listrik.id_kabupaten,data.listrik.id_kecamatan)
				// getKelurahan(data.listrik.id_kecamatan,data.listrik.id_kelurahan)

				if(data.listrik.flag_location == 'prov')
				{
					getProvinsi(data.listrik.id_provinsi)
					getKabupaten(data.listrik.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.listrik.flag_location == 'kab')
				{
					getProvinsi(data.listrik.id_provinsi)
					getKabupaten(data.listrik.id_provinsi,data.listrik.id_kabupaten)
					getKecamatan(data.listrik.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.listrik.flag_location == 'kec')
				{
					getProvinsi(data.listrik.id_provinsi)
					getKabupaten(data.listrik.id_provinsi,data.listrik.id_kabupaten)
					getKecamatan(data.listrik.id_kabupaten,data.listrik.id_kecamatan)
					getKelurahan(data.listrik.id_kecamatan,0)
				}
				else if(data.listrik.flag_location == 'kel')
				{
					getProvinsi(data.listrik.id_provinsi)
					getKabupaten(data.listrik.id_provinsi,data.listrik.id_kabupaten)
					getKecamatan(data.listrik.id_kabupaten,data.listrik.id_kecamatan)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>geografi_sumberListrik/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
