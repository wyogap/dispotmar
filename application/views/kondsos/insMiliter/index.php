<div class="section">
	...
	<div class="page-header">
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active" aria-current="page">Instansi Militer dan Polisi</li>
		</ol>
	</div>
	<!-- Page-header closed -->
		...
		<div class="col-md-12 col-lg-12">
			<div class="card" style="overflow:auto;">
				<div class="card-header">
					<div class="card-title">Instansi Militer dan Polisi</div>
				...
				<div class="card-body">
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataMiliter as $militer): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('KONSOS','update')): ?>
										<button onclick="editModal(`<?= encrypt($militer->id_instansi_militer); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($militer->id_instansi_militer); ?>`,'<?= $militer->instansi ?>')"
									...
									<td><?= $no++ ?></td>
									<td><?= $militer->nama_satker ?></td>
									<td><?= $militer->wilayah ?></td>
									<td><?= $militer->instansi ?></td>
									<td><?= $militer->cakupan_wilayah ?></td>
									<td><?= $militer->jumlah_personel ?></td>
									<td><?= $militer->keterangan ?></td>
									<td><?= $militer->nama_pegawai ?></td>
									<td><?= $militer->LastUpdated ?></td>
								</tr>
								<?php endforeach ?>
							</tbody>
						</table>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<!-- <div class="form-group row">
								<label class="col-md-3 col-form-label" for="instansi">Instansi</label>
								<div class="col-md-9">
									<input type="text" id="" name="instansi" class="form-control">
									<div class="invalid-feedback warning-instansi"></div>
								</div>
							</div> -->
							<div class="form-group row">
								<label class="col-md-3 col-form-label">Instansi</label>
								<div class="col-md-9">
									<select class="form-control" name="instansi" id="instansi" style="width: 100%;">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="cakupan">Cakupan Wilayah</label>
								<div class="col-md-9">
									<input type="text" id="" name="cakupan" class="form-control">
									<div class="invalid-feedback warning-cakupan"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="jumlah">Jumlah Personil</label>
								<div class="col-md-9">
					...
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label">Instansi</label>
								<div class="col-md-9">
									<select class="form-control" name="instansi" id="instansiEdit" style="width: 100%;">
										<?php foreach($instansis as $instansi): ?>
										<option value="<?= $instansi->id_instansi?>"><?= $instansi->nama ?></option>
										<?php endforeach ?>
									</select>
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/kondsos_insMiliter/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>kondsos_insMiliters/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>kondsos_insMiliter/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('select[name=instansi]').find('option:selected').removeAttr('selected');
				$('input[name="id"]').val(id);
				$('input[name="satker"]').val(data.militer.id_satker);
				$('input[name="cakupan"]').val(data.militer.cakupan_wilayah);
				$('input[name="jumlah"]').val(data.militer.jumlah_personel);
				$('input[name="kebudayaan"]').val(data.militer.kebudayaan_asli);
				$('input[name="notes"]').val(data.militer.keterangan);
				$('input[name="flag_locationedit"]').val(data.militer.flag_location);
				//$("select[name=satker] option[value="+data.militer.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.militer.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.militer.id_provinsi+"]").attr('selected','selected');
				//$("select[name=instansi] option[value="+data.militer.id_instansi+"]").attr('selected','selected');

				$("#satkerEdit").val(data.militer.id_satker);
				$("#provinsiEdit").val(data.militer.id_provinsi);
				$("#instansiEdit").val(data.militer.id_instansi);
				...
				$("#instansiEdit").trigger('change');

				// getProvinsi(data.militer.id_provinsi)
				// getKabupaten(data.militer.id_provinsi,data.militer.id_kabupaten)
				// getKecamatan(data.militer.id_kabupaten,data.militer.id_kecamatan)
				// getKelurahan(data.militer.id_kecamatan,data.militer.id_kelurahan)

				if(data.militer.flag_location == 'prov')
				{
					getProvinsi(data.militer.id_provinsi)
					getKabupaten(data.militer.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.militer.flag_location == 'kab')
				{
					getProvinsi(data.militer.id_provinsi)
					getKabupaten(data.militer.id_provinsi,data.militer.id_kabupaten)
					getKecamatan(data.militer.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.militer.flag_location == 'kec')
				{
					getProvinsi(data.militer.id_provinsi)
					getKabupaten(data.militer.id_provinsi,data.militer.id_kabupaten)
					getKecamatan(data.militer.id_kabupaten,data.militer.id_kecamatan)
					getKelurahan(data.militer.id_kecamatan,0)
				}
				else if(data.militer.flag_location == 'kel')
				{
					getProvinsi(data.militer.id_provinsi)
					getKabupaten(data.militer.id_provinsi,data.militer.id_kabupaten)
					getKecamatan(data.militer.id_kabupaten,data.militer.id_kecamatan)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>kondsos_insMiliter/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
