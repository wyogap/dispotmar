<div class="section">
	...
	<div class="page-header">
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active" aria-current="page">Organisasi Agama</li>
		</ol>
	</div>
	<!-- Page-header closed -->
		...
		<div class="col-md-12 col-lg-12">
			<div class="card" style="overflow:auto;">
				<div class="card-header">
					<div class="card-title">Organisasi Agama</div>
							...
							<tbody>
								...
								<tr>
									<td class="text-center">
										...
										<button onclick="editModal(`<?= encrypt($agama->id_organisasi_agama); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($agama->id_organisasi_agama); ?>`,'<?= $agama->nama_organisasi ?>')"
									...
									<td><?= $agama->wilayah ?></td>
									<td><?= $agama->nama_organisasi ?></td>
									<td><?= $agama->alamat_kantor_pusat ?></td>
									<td><?= $agama->agama ?></td>
									<td><?= $agama->pemimpin ?></td>
									<td><?= $agama->jumlah_anggota ?></td>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="leader">Pemimpin</label>
								<div class="col-md-9">
									<input type="text" id="" name="leader" class="form-control">
									<div class="invalid-feedback warning-leader"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="total">Jumlah Anggota (Orang)</label>
								<div class="col-md-9">
									<input type="number" id="" name="total" class="form-control">
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/kondsos_orgAgama/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>kondsos_orgAgamas/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>kondsos_orgAgama/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('input[name="name"]').val(data.agama.nama_organisasi);
				$('input[name="address"]').val(data.agama.alamat_kantor_pusat);
				$('input[name="leader"]').val(data.agama.pemimpin);
				$('input[name="total"]').val(data.agama.jumlah_anggota);
				...
				//$("select[name=religion] option[value="+data.agama.id_jenis_agama+"]").attr('selected','selected');

				$("#satkerEdit").val(data.agama.id_satker);
				$("#provinsiEdit").val(data.agama.id_provinsi);
				$("#religionEdit").val(data.agama.id_jenis_agama);
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>kondsos_orgAgama/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
