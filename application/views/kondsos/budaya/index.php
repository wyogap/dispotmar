		...
		<div class="col-md-12 col-lg-12">
			<div class="card" style="overflow:auto;">
				<div class="card-header">
					<div class="card-title">Budaya</div>
				...
				<div class="card-body">
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataBudaya as $budaya): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('KONSOS','update')): ?>
										<button onclick="editModal(`<?= encrypt($budaya->id_budaya); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($budaya->id_budaya); ?>`,'<?= $budaya->kebudayaan_asli ?>')"
									...
									<td><?= $no++ ?></td>
									<td><?= $budaya->nama_satker ?></td>
									<td><?= $budaya->wilayah ?></td>
									<td><?= $budaya->kebudayaan_asli ?></td>
									<td><?= $budaya->keterangan ?></td>
									<td><?= $budaya->nama_pegawai ?></td>
									<td><?= $budaya->LastUpdated ?></td>
								</tr>
								<?php endforeach ?>
							</tbody>
						</table>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="kebudayaan">Kebudayaan Asli</label>
								<div class="col-md-9">
									<input type="text" id="" name="kebudayaan" class="form-control">
									<div class="invalid-feedback warning-kebudayaan"></div>
								</div>
							</div>
							<div class="form-group row">
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/kondsos_budaya/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>kondsos_budayas/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>kondsos_budaya/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('input[name="satker"]').val(data.budaya.id_satker);
				$('input[name="kebudayaan"]').val(data.budaya.kebudayaan_asli);
				$('input[name="notes"]').val(data.budaya.keterangan);
				$('input[name="flag_locationedit"]').val(data.budaya.flag_location);
				//$("select[name=satker] option[value="+data.budaya.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.budaya.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.budaya.id_provinsi+"]").attr('selected','selected');

				$("#satkerEdit").val(data.budaya.id_satker);
				$("#provinsiEdit").val(data.budaya.id_provinsi);
				...
				// getProvinsi(data.budaya.id_provinsi)
				// getKabupaten(data.budaya.id_provinsi,data.budaya.id_kabupaten)
				// getKecamatan(data.budaya.id_kabupaten,data.budaya.id_kecamatan)
				// getKelurahan(data.budaya.id_kecamatan,data.budaya.id_kelurahan)

				if(data.budaya.flag_location == 'prov')
				{
					getProvinsi(data.budaya.id_provinsi)
					getKabupaten(data.budaya.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.budaya.flag_location == 'kab')
				{
					getProvinsi(data.budaya.id_provinsi)
					getKabupaten(data.budaya.id_provinsi,data.budaya.id_kabupaten)
					getKecamatan(data.budaya.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.budaya.flag_location == 'kec')
				{
					getProvinsi(data.budaya.id_provinsi)
					getKabupaten(data.budaya.id_provinsi,data.budaya.id_kabupaten)
					getKecamatan(data.budaya.id_kabupaten,data.budaya.id_kecamatan)
					getKelurahan(data.budaya.id_kecamatan,0)
				}
				else if(data.budaya.flag_location == 'kel')
				{
					getProvinsi(data.budaya.id_provinsi)
					getKabupaten(data.budaya.id_provinsi,data.budaya.id_kabupaten)
					getKecamatan(data.budaya.id_kabupaten,data.budaya.id_kecamatan)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>kondsos_budaya/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
