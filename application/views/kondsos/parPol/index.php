		...
		<div class="col-md-12 col-lg-12">
			<div class="card" style="overflow:auto;">
				<div class="card-header">
					<div class="card-title">Partai Politik</div>
				...
				<div class="card-body">
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataParpol as $parpol): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('KONSOS','update')): ?>
										<button onclick="editModal(`<?= encrypt($parpol->id_partai_politik); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($parpol->id_partai_politik); ?>`,'<?= $parpol->partai ?>')"
									...
									<td><?= $no++ ?></td>
									<td><?= $parpol->nama_satker ?></td>
									<td><?= $parpol->wilayah ?></td>
									<td><?= $parpol->partai ?></td>
									<td><?= $parpol->prosentase ?></td>
									<td><?= $parpol->dominasi_wilayah ?></td>
									<td><?= $parpol->keterangan ?></td>
									<td><?= $parpol->nama_pegawai ?></td>
									<td><?= $parpol->LastUpdated ?></td>
								</tr>
								<?php endforeach ?>
							</tbody>
						</table>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="prosentase">Prosentase (%)</label>
								<div class="col-md-9">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="dominasi">Dominasi Wilayah</label>
								<div class="col-md-9">
									<input type="text" id="" name="dominasi" class="form-control">
									<div class="invalid-feedback warning-dominasi"></div>
								</div>
							</div>
							<div class="form-group row">
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/kondsos_parPol/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>kondsos_parPols/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>kondsos_parPol/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('input[name="satker"]').val(data.parpol.id_satker);
				$('input[name="prosentase"]').val(data.parpol.prosentase);
				$('input[name="dominasi"]').val(data.parpol.dominasi_wilayah);
				$('input[name="notes"]').val(data.parpol.keterangan);
				$('input[name="flag_locationedit"]').val(data.parpol.flag_location);
				//$("select[name=satker] option[value="+data.parpol.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.parpol.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.parpol.id_provinsi+"]").attr('selected','selected');
				//$("select[name=partai] option[value="+data.parpol.id_partai+"]").attr('selected','selected');

				$("#satkerEdit").val(data.parpol.id_satker);
				$("#provinsiEdit").val(data.parpol.id_provinsi);
				$("#partaiEdit").val(data.parpol.id_partai);
				...
				// getProvinsi(data.parpol.id_provinsi)
				// getKabupaten(data.parpol.id_provinsi,data.parpol.id_kabupaten)
				// getKecamatan(data.parpol.id_kabupaten,data.parpol.id_kecamatan)
				// getKelurahan(data.parpol.id_kecamatan,data.parpol.id_kelurahan)

				if(data.parpol.flag_location == 'prov')
				{
					getProvinsi(data.parpol.id_provinsi)
					getKabupaten(data.parpol.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.parpol.flag_location == 'kab')
				{
					getProvinsi(data.parpol.id_provinsi)
					getKabupaten(data.parpol.id_provinsi,data.parpol.id_kabupaten)
					getKecamatan(data.parpol.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.parpol.flag_location == 'kec')
				{
					getProvinsi(data.parpol.id_provinsi)
					getKabupaten(data.parpol.id_provinsi,data.parpol.id_kabupaten)
					getKecamatan(data.parpol.id_kabupaten,data.parpol.id_kecamatan)
					getKelurahan(data.parpol.id_kecamatan,0)
				}
				else if(data.parpol.flag_location == 'kel')
				{
					getProvinsi(data.parpol.id_provinsi)
					getKabupaten(data.parpol.id_provinsi,data.parpol.id_kabupaten)
					getKecamatan(data.parpol.id_kabupaten,data.parpol.id_kecamatan)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>kondsos_parPol/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
