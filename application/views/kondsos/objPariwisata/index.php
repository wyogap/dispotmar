<div class="section">
	...
	<div class="page-header">
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active" aria-current="page">Objek Pariwisata</li>
		</ol>
	</div>
	<!-- Page-header closed -->
		...
		<div class="col-md-12 col-lg-12">
			<div class="card" style="overflow:auto;">
				<div class="card-header">
					<div class="card-title">Objek Pariwisata</div>
				...
				<div class="card-body">
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataPariwisata as $pariwisata): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('KONSOS','update')): ?>
										<button onclick="editModal(`<?= encrypt($pariwisata->id_objek_pariwisata); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($pariwisata->id_objek_pariwisata); ?>`,'<?= $pariwisata->objek_pariwisata ?>')"
									...
									<td><?= $no++ ?></td>
									<td><?= $pariwisata->nama_satker ?></td>
									<td><?= $pariwisata->wilayah ?></td>
									<td><?= $pariwisata->objek_pariwisata ?></td>
									<td><?= $pariwisata->alamat ?></td>
									<td><?= $pariwisata->Pengelola ?></td>
									<td><?= $pariwisata->keterangan ?></td>
									<td><?= $pariwisata->nama_pegawai ?></td>
									<td><?= $pariwisata->LastUpdated ?></td>
								</tr>
								<?php endforeach ?>
							</tbody>
						</table>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="pariwisata">Objek Pariwisata</label>
								<div class="col-md-9">
									<input type="text" id="" name="pariwisata" class="form-control">
									<div class="invalid-feedback warning-pariwisata"></div>
								</div>
							</div>
							<div class="form-group row">
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/kondsos_objPariwisata/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>kondsos_objPariwisatas/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>kondsos_objPariwisata/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('input[name="pengelola"]').val(data.pariwisata.Pengelola);
				$('input[name="id"]').val(id);
				$('input[name="satker"]').val(data.pariwisata.id_satker);
				$('input[name="pariwisata"]').val(data.pariwisata.objek_pariwisata);
				$('input[name="alamat"]').val(data.pariwisata.alamat);
				$('input[name="notes"]').val(data.pariwisata.keterangan);
				$('input[name="flag_locationedit"]').val(data.pariwisata.flag_location);
				//$("select[name=satker] option[value="+data.pariwisata.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.pariwisata.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.pariwisata.id_provinsi+"]").attr('selected','selected');
				// $("select[name=pengelola] option[value="+data.pariwisata.id_pengelola+"]").attr('selected','selected');

				$("#satkerEdit").val(data.pariwisata.id_satker);
				$("#provinsiEdit").val(data.pariwisata.id_provinsi);
				...
				// getProvinsi(data.pariwisata.id_provinsi)
				// getKabupaten(data.pariwisata.id_provinsi,data.pariwisata.id_kabupaten)
				// getKecamatan(data.pariwisata.id_kabupaten,data.pariwisata.id_kecamatan)
				// getKelurahan(data.pariwisata.id_kecamatan,data.pariwisata.id_kelurahan)

				if(data.pariwisata.flag_location == 'prov')
				{
					getProvinsi(data.pariwisata.id_provinsi)
					getKabupaten(data.pariwisata.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.pariwisata.flag_location == 'kab')
				{
					getProvinsi(data.pariwisata.id_provinsi)
					getKabupaten(data.pariwisata.id_provinsi,data.pariwisata.id_kabupaten)
					getKecamatan(data.pariwisata.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.pariwisata.flag_location == 'kec')
				{
					getProvinsi(data.pariwisata.id_provinsi)
					getKabupaten(data.pariwisata.id_provinsi,data.pariwisata.id_kabupaten)
					getKecamatan(data.pariwisata.id_kabupaten,data.pariwisata.id_kecamatan)
					getKelurahan(data.pariwisata.id_kecamatan,0)
				}
				else if(data.pariwisata.flag_location == 'kel')
				{
					getProvinsi(data.pariwisata.id_provinsi)
					getKabupaten(data.pariwisata.id_provinsi,data.pariwisata.id_kabupaten)
					getKecamatan(data.pariwisata.id_kabupaten,data.pariwisata.id_kecamatan)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>kondsos_objPariwisata/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
