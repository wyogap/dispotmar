<div class="section">
	...
	<div class="page-header">
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active" aria-current="page">Industri UMKM</li>
		</ol>
	</div>
	<!-- Page-header closed -->
		...
		<div class="col-md-12 col-lg-12">
			<div class="card" style="overflow:auto;">
				<div class="card-header">
					<div class="card-title">Industri UMKM</div>
							...
							<tbody>
								...
								<tr>
									<td class="text-center">
										...
										<button onclick="editModal(`<?= encrypt($umkm->id_umkm); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($umkm->id_umkm); ?>`,'<?= $umkm->jenis_industri ?>')"
									...
									<td><?= $no++ ?></td>
									<td><?= $umkm->nama_satker ?></td>
									<td><?= $umkm->wilayah ?></td>
									<td><?= $umkm->jenis_industri ?></td>
									<td><?= $umkm->penjualan ?></td>
									<td><?= $umkm->jumlah_tenaga_kerja ?></td>
									<td><?= $umkm->keterangan ?></td>
									<td><?= $umkm->nama_pegawai ?></td>
									<td><?= $umkm->LastUpdated ?></td>
								</tr>
								<?php endforeach ?>
							</tbody>
						</table>
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/kondsos_indUmkm/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>kondsos_indUmkms/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>kondsos_indUmkm/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('input[name="satker"]').val(data.umkm.id_satker);
				$('input[name="jenis"]').val(data.umkm.jenis_industri);
				$('input[name="penjualan"]').val(data.umkm.penjualan);
				$('input[name="jumlah"]').val(data.umkm.jumlah_tenaga_kerja);
				$('input[name="notes"]').val(data.umkm.keterangan);
				$('input[name="flag_locationedit"]').val(data.umkm.flag_location);
				$("select[name=satker] option[value="+data.umkm.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.umkm.id_satker + "]").attr('selected','selected');
				$("select[name=provinsi] option[value="+data.umkm.id_provinsi+"]").attr('selected','selected');

				$("#satkerEdit").val(data.umkm.id_satker);
				$("#provinsiEdit").val(data.umkm.id_provinsi);
				...
				// getProvinsi(data.umkm.id_provinsi)
				// getKabupaten(data.umkm.id_provinsi,data.umkm.id_kabupaten)
				// getKecamatan(data.umkm.id_kabupaten,data.umkm.id_kecamatan)
				// getKelurahan(data.umkm.id_kecamatan,data.umkm.id_kelurahan)

				if(data.umkm.flag_location == 'prov')
				{
					getProvinsi(data.umkm.id_provinsi)
					getKabupaten(data.umkm.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.umkm.flag_location == 'kab')
				{
					getProvinsi(data.umkm.id_provinsi)
					getKabupaten(data.umkm.id_provinsi,data.umkm.id_kabupaten)
					getKecamatan(data.umkm.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.umkm.flag_location == 'kec')
				{
					getProvinsi(data.umkm.id_provinsi)
					getKabupaten(data.umkm.id_provinsi,data.umkm.id_kabupaten)
					getKecamatan(data.umkm.id_kabupaten,data.umkm.id_kecamatan)
					getKelurahan(data.umkm.id_kecamatan,0)
				}
				else if(data.umkm.flag_location == 'kel')
				{
					getProvinsi(data.umkm.id_provinsi)
					getKabupaten(data.umkm.id_provinsi,data.umkm.id_kabupaten)
					getKecamatan(data.umkm.id_kabupaten,data.umkm.id_kecamatan)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>kondsos_indUmkm/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
