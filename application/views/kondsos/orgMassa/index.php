<div class="section">
	...
	<div class="page-header">
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active" aria-current="page">Organisasi Massa</li>
		</ol>
	</div>
	<!-- Page-header closed -->
		...
		<div class="col-md-12 col-lg-12">
			<div class="card" style="overflow:auto;">
				<div class="card-header">
					<div class="card-title">Organisasi Massa</div>
				...
				<div class="card-body">
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataMasa as $masa): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('KONSOS','update')): ?>
										<button onclick="editModal(`<?= encrypt($masa->id_organisasi_massa); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($masa->id_organisasi_massa); ?>`,'<?= $masa->nama_organisasi ?>')"
									...
									<td><?= $no++ ?></td>
									<td><?= $masa->nama_satker ?></td>
									<td><?= $masa->wilayah ?></td>
									<td><?= $masa->nama_organisasi ?></td>
									<td><?= $masa->alamat_kantor_pusat ?></td>
									<td><?= $masa->tertua ?></td>
									<td><?= $masa->bidang ?></td>
									<td><?= $masa->keterangan ?></td>
									<td><?= $masa->nama_pegawai ?></td>
									<td><?= $masa->LastUpdated ?></td>
								</tr>
								<?php endforeach ?>
							</tbody>
						</table>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="bidang">Bidang</label>
								<div class="col-md-9">
									<input type="text" id="" name="bidang" class="form-control">
									<div class="invalid-feedback warning-bidang"></div>
								</div>
							</div>
							<div class="form-group row">
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/kondsos_orgMassa/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>kondsos_orgMassas/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>kondsos_orgMassa/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('input[name="satker"]').val(data.masa.id_satker);
				$('input[name="name"]').val(data.masa.nama_organisasi);
				$('input[name="address"]').val(data.masa.alamat_kantor_pusat);
				$('input[name="tertua"]').val(data.masa.tertua);
				$('input[name="bidang"]').val(data.masa.bidang);
				$('input[name="notes"]').val(data.masa.keterangan);
				$('input[name="flag_locationedit"]').val(data.masa.flag_location);
				//$("select[name=satker] option[value="+data.masa.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.masa.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.masa.id_provinsi+"]").attr('selected','selected');

				$("#satkerEdit").val(data.masa.id_satker);
				$("#provinsiEdit").val(data.masa.id_provinsi);
				...
				// getProvinsi(data.masa.id_provinsi)
				// getKabupaten(data.masa.id_provinsi,data.masa.id_kabupaten)
				// getKecamatan(data.masa.id_kabupaten,data.masa.id_kecamatan)
				// getKelurahan(data.masa.id_kecamatan,data.masa.id_kelurahan)

				if(data.masa.flag_location == 'prov')
				{
					getProvinsi(data.masa.id_provinsi)
					getKabupaten(data.masa.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.masa.flag_location == 'kab')
				{
					getProvinsi(data.masa.id_provinsi)
					getKabupaten(data.masa.id_provinsi,data.masa.id_kabupaten)
					getKecamatan(data.masa.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.masa.flag_location == 'kec')
				{
					getProvinsi(data.masa.id_provinsi)
					getKabupaten(data.masa.id_provinsi,data.masa.id_kabupaten)
					getKecamatan(data.masa.id_kabupaten,data.masa.id_kecamatan)
					getKelurahan(data.masa.id_kecamatan,0)
				}
				else if(data.masa.flag_location == 'kel')
				{
					getProvinsi(data.masa.id_provinsi)
					getKabupaten(data.masa.id_provinsi,data.masa.id_kabupaten)
					getKecamatan(data.masa.id_kabupaten,data.masa.id_kecamatan)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>kondsos_orgMassa/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
