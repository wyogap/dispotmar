<div class="section">
	...
	<div class="page-header">
		...
		<li class="breadcrumb-item"><a href="#"><i class="ti-package mr-1"></i>Kondisi Sosial</a></li>
			<li class="breadcrumb-item active" aria-current="page">Peninggalan Sejarah</li>
		</ol>
	</div>
	<!-- Page-header closed -->
		...
		<div class="col-md-12 col-lg-12">
			<div class="card" style="overflow:auto;">
				<div class="card-header">
					<div class="card-title">Peninggalan Sejarah</div>
				...
				<div class="card-body">
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataSejarah as $sejarah): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('KONSOS','update')): ?>
										<button onclick="editModal(`<?= encrypt($sejarah->id_peninggalan_sejarah); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($sejarah->id_peninggalan_sejarah); ?>`,'<?= $sejarah->objek_sejarah ?>')"
									...
									<td><?= $no++ ?></td>
									<td><?= $sejarah->nama_satker ?></td>
									<td><?= $sejarah->wilayah ?></td>
									<td><?= $sejarah->objek_sejarah ?></td>
									<td><?= $sejarah->titik_kordinat ?></td>
									<td><?= $sejarah->pengelola ?></td>
									<td><?= $sejarah->keterangan ?></td>
									<td><?= $sejarah->nama_pegawai ?></td>
									<td><?= $sejarah->LastUpdated ?></td>
								</tr>
								<?php endforeach ?>
							</tbody>
						</table>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="sejarah">Objek Sejarah</label>
								<div class="col-md-9">
									<input type="text" id="" name="sejarah" class="form-control">
									<div class="invalid-feedback warning-sejarah"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="koordinat">Titik Koordinat</label>
								<div class="col-md-9">
									<input type="text" id="" name="koordinat" class="form-control">
									<div class="invalid-feedback warning-koordinat"></div>
								</div>
							</div>
							<!-- <div class="form-group row">
								<label class="col-md-3 col-form-label" for="pengelola">Pengelola</label>
								<div class="col-md-9">
									<input type="text" id="" name="pengelola" class="form-control">
									<div class="invalid-feedback warning-pengelola"></div>
								</div>
							</div> -->
							<div class="form-group row">
								<label class="col-md-3 col-form-label">Pengelola</label>
								<div class="col-md-9">
									<select class="form-control" name="pengelola" id="pengelola" style="width: 100%;">
										<?php foreach($pengelolas as $pengelola): ?>
										<option value="<?= $pengelola->id_pengelola?>"><?= $pengelola->nama ?></option>
										<?php endforeach ?>
									</select>
									<div class="invalid-feedback warning-pengelola"></div>
								</div>
					...
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label">Pengelola</label>
								<div class="col-md-9">
									<select class="form-control" name="pengelola" id="pengelolaEdit" style="width: 100%;">
										<?php foreach($pengelolas as $pengelola): ?>
										<option value="<?= $pengelola->id_pengelola?>"><?= $pengelola->nama ?></option>
										<?php endforeach ?>
									</select>
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/kondsos_penSejarah/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>kondsos_penSejarahs/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>kondsos_penSejarah/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('select[name=pengelola]').find('option:selected').removeAttr('selected');
				$('input[name="id"]').val(id);
				$('input[name="satker"]').val(data.sejarah.id_satker);
				$('input[name="sejarah"]').val(data.sejarah.objek_sejarah);
				$('input[name="koordinat"]').val(data.sejarah.titik_kordinat);
				$('input[name="notes"]').val(data.sejarah.keterangan);
				$('input[name="flag_locationedit"]').val(data.sejarah.flag_location);
				//$("select[name=satker] option[value="+data.sejarah.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.sejarah.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.sejarah.id_provinsi+"]").attr('selected','selected');
				//$("select[name=pengelola] option[value="+data.sejarah.id_pengelola+"]").attr('selected','selected');

				$("#satkerEdit").val(data.sejarah.id_satker);
				$("#provinsiEdit").val(data.sejarah.id_provinsi);
				$("#pengelolaEdit").val(data.sejarah.id_pengelola);
				...
				$("#pengelolaEdit").trigger('change');

				// getProvinsi(data.sejarah.id_provinsi)
				// getKabupaten(data.sejarah.id_provinsi,data.sejarah.id_kabupaten)
				// getKecamatan(data.sejarah.id_kabupaten,data.sejarah.id_kecamatan)
				// getKelurahan(data.sejarah.id_kecamatan,data.sejarah.id_kelurahan)

				if(data.sejarah.flag_location == 'prov')
				{
					getProvinsi(data.sejarah.id_provinsi)
					getKabupaten(data.sejarah.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.sejarah.flag_location == 'kab')
				{
					getProvinsi(data.sejarah.id_provinsi)
					getKabupaten(data.sejarah.id_provinsi,data.sejarah.id_kabupaten)
					getKecamatan(data.sejarah.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.sejarah.flag_location == 'kec')
				{
					getProvinsi(data.sejarah.id_provinsi)
					getKabupaten(data.sejarah.id_provinsi,data.sejarah.id_kabupaten)
					getKecamatan(data.sejarah.id_kabupaten,data.sejarah.id_kecamatan)
					getKelurahan(data.sejarah.id_kecamatan,0)
				}
				else if(data.sejarah.flag_location == 'kel')
				{
					getProvinsi(data.sejarah.id_provinsi)
					getKabupaten(data.sejarah.id_provinsi,data.sejarah.id_kabupaten)
					getKecamatan(data.sejarah.id_kabupaten,data.sejarah.id_kecamatan)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>kondsos_penSejarah/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
