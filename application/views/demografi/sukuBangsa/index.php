<div class="section">
	...
	<div class="page-header">
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active" aria-current="page">Suku Bangsa</li>
		</ol>
	</div>
	<!-- Page-header closed -->
		...
		<div class="col-md-12 col-lg-12">
			<div class="card" style="overflow:auto;">
				<div class="card-header">
					<div class="card-title">Suku Bangsa</div>
				...
				<div class="card-body">
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataSuku as $suku): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('DEMO','update')): ?>
										<button onclick="editModal(`<?= encrypt($suku->id_suku); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($suku->id_suku); ?>`,'<?= $suku->jenis_suku ?>')"
									...
									<td><?= $no++ ?></td>
									<td><?= $suku->nama_satker ?></td>
									<td><?= $suku->wilayah ?></td>
									<td><?= $suku->jenis_suku ?></td>
									<td><?= $suku->prosentase ?></td>
									<td><?= $suku->ciri_khas ?></td>
									<td><?= $suku->bahasa_adat ?></td>
									<td><?= $suku->tertua_adat ?></td>
									<td><?= $suku->keterangan ?></td>
									<td><?= $suku->nama_pegawai ?></td>
									<td><?= $suku->LastUpdated ?></td>
								</tr>
								<?php endforeach ?>
							</tbody>
						</table>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<!-- <div class="form-group row">
								<label class="col-md-3 col-form-label" for="suku">Suku</label>
								<div class="col-md-9">
									<input type="text" id="" name="suku" class="form-control">
									<div class="invalid-feedback warning-suku"></div>
								</div>
							</div> -->
							<div class="form-group row">
								<label class="col-md-3 col-form-label">Suku Bangsa</label>
								<div class="col-md-9">
									<select class="form-control" name="suku" id="suku" style="width: 100%;">
										<?php foreach($jenis_suku as $suku): ?>
											<option value="<?= $suku->id_jenis_suku ?>"><?= $suku->nama ?></option>
										<?php endforeach?>
									</select>
									<div class="invalid-feedback warning-suku"></div>
								</div>
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="prosentase">Persentase (%)</label>
								<div class="col-md-9">
									<input type="text" onkeypress="return isNumberKey(event)" step="any" id="" name="prosentase" class="form-control">
									<div class="invalid-feedback warning-prosentase"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="ciri_khas">Ciri Khas</label>
								<div class="col-md-9">
									<input type="text" id="" name="ciri_khas" class="form-control">
									<div class="invalid-feedback warning-ciri_khas"></div>
								</div>
							</div>
							<!-- <div class="form-group row">
								<label class="col-md-3 col-form-label" for="bahasa_adat">Bahasa Adat</label>
								<div class="col-md-9">
									<input type="text" id="" name="bahasa_adat" class="form-control">
									<div class="invalid-feedback warning-bahasa_adat"></div>
								</div>
							</div> -->
							<div class="form-group row">
								<label class="col-md-3 col-form-label">Bahasa Adat</label>
								<div class="col-md-9">
									<select class="form-control" name="bahasa_adat" id="bahasa_adat" style="width: 100%;">
										<?php foreach($bahasa_adat as $bahasa): ?>
											<option value="<?= $bahasa->id_bahasa_adat ?>"><?= $bahasa->nama ?></option>
										<?php endforeach?>
									</select>
									<div class="invalid-feedback warning-bahasa_adat"></div>
								</div>
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="tertua_adat">Tertua Adat</label>
								<div class="col-md-9">
									<input type="text" id="" name="tertua_adat" class="form-control">
									<div class="invalid-feedback warning-tertua_adat"></div>
								</div>
							</div>
							<div class="form-group row">
					...
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label">Suku Bangsa</label>
								<div class="col-md-9">
									<select class="form-control" name="suku" id="sukuEdit" style="width: 100%;">
										<?php foreach($jenis_suku as $suku): ?>
											<option value="<?= $suku->id_jenis_suku ?>"><?= $suku->nama ?></option>
										<?php endforeach?>
									</select>
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label">Bahasa Adat</label>
								<div class="col-md-9">
									<select class="form-control" name="bahasa_adat" id="bahasa_adatEdit" style="width: 100%;">
										<?php foreach($bahasa_adat as $bahasa): ?>
											<option value="<?= $bahasa->id_bahasa_adat ?>"><?= $bahasa->nama ?></option>
										<?php endforeach?>
									</select>
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/demografi_sukuBangsa/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>demografi_sukuBangsas/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>demografi_sukuBangsa/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('select[name=bahasa_adat]').find('option:selected').removeAttr('selected');
				$('input[name="id"]').val(id);
				$('input[name="satker"]').val(data.suku.id_satker);
				$('input[name="suku"]').val(data.suku.suku);
				$('input[name="prosentase"]').val(data.suku.prosentase);
				$('input[name="ciri_khas"]').val(data.suku.ciri_khas);
				$('input[name="tertua_adat"]').val(data.suku.tertua_adat);
				$('input[name="notes"]').val(data.suku.keterangan);
				$('input[name="flag_locationedit"]').val(data.suku.flag_location);
				//$("select[name=satker] option[value="+data.suku.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.suku.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.suku.id_provinsi+"]").attr('selected','selected');
				//$("select[name=bahasa_adat] option[value="+data.suku.id_bahasa_adat+"]").attr('selected','selected');

				$("#satkerEdit").val(data.suku.id_satker);
				$("#provinsiEdit").val(data.suku.id_provinsi);
				$("#sukuEdit").val(data.suku.id_jenis_suku);
				$("#bahasa_adatEdit").val(data.suku.id_bahasa_adat);
				...
				$("#sukuEdit").trigger('change');
				$("#bahasa_adatEdit").trigger('change');

				// getProvinsi(data.suku.id_provinsi)
				// getKabupaten(data.suku.id_provinsi,data.suku.id_kabupaten)
				// getKecamatan(data.suku.id_kabupaten,data.suku.id_kecamatan)
				// getKelurahan(data.suku.id_kecamatan,data.suku.id_kelurahan)

				if(data.suku.flag_location == 'prov')
				{
					getProvinsi(data.suku.id_provinsi)
					getKabupaten(data.suku.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.suku.flag_location == 'kab')
				{
					getProvinsi(data.suku.id_provinsi)
					getKabupaten(data.suku.id_provinsi,data.suku.id_kabupaten)
					getKecamatan(data.suku.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.suku.flag_location == 'kec')
				{
					getProvinsi(data.suku.id_provinsi)
					getKabupaten(data.suku.id_provinsi,data.suku.id_kabupaten)
					getKecamatan(data.suku.id_kabupaten,data.suku.id_kecamatan)
					getKelurahan(data.suku.id_kecamatan,0)
				}
				else if(data.suku.flag_location == 'kel')
				{
					getProvinsi(data.suku.id_provinsi)
					getKabupaten(data.suku.id_provinsi,data.suku.id_kabupaten)
					getKecamatan(data.suku.id_kabupaten,data.suku.id_kecamatan)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>demografi_sukuBangsa/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
