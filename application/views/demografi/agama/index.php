			...
			<div class="card" style="overflow:auto;">
				...
				<div class="card-body">
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataAgama as $agama): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('DEMO','update')): ?>
										<button onclick="editModal(`<?= encrypt($agama->id_agama); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($agama->id_agama); ?>`,'Agama')"
									...
									<td><?= $no++ ?></td>
									<td><?= $agama->nama_satker ?></td>
									<td><?= $agama->wilayah ?></td>
									<td><?= $agama->agama ?></td>
									<td><?= $agama->prosentase ?></td>
									<td><?= $agama->jumlah_tempat_ibadah ?></td>
									<td><?= $agama->keterangan ?></td>
									<td><?= $agama->nama_pegawai ?></td>
									<td><?= $agama->LastUpdated ?></td>
								</tr>
								<?php endforeach ?>
							</tbody>
						</table>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="agama">Agama</label>
								<div class="col-md-9">
									...
									<div class="invalid-feedback warning-agama"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="prosentase">%</label>
								<div class="col-md-9">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="tempat_ibadah">Jumlah Tempat Ibadah (Buah)</label>
								<div class="col-md-9">
									<input type="number" id="" name="tempat_ibadah" class="form-control">
									<div class="invalid-feedback warning-tempat_ibadah"></div>
								</div>
							</div>
							<div class="form-group row">
					...
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="agama">Agama</label>
								<div class="col-md-9">
									<select class="form-control" name="agama" id="agamaEdit" style="width: 100%;">
										<option value="">Pilih Agama</option>
										<?php foreach($agamas as $agama): ?>
											<option value="<?= $agama->id_jenis_agama ?>"><?= $agama->nama ?></option>
										<?php endforeach?>
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/demografi_agama/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>demografi_agamas/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>demografi_agama/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('select[name=agama]').find('option:selected').removeAttr('selected');
				$('input[name="id"]').val(id);
				$('input[name="satker"]').val(data.agama.id_satker);
				$('input[name="jenis_agama"]').val(data.agama.id_jenis_agama);
				$('input[name="prosentase"]').val(data.agama.prosentase);
				$('input[name="tempat_ibadah"]').val(data.agama.jumlah_tempat_ibadah);
				$('input[name="notes"]').val(data.agama.keterangan);
				$('input[name="flag_locationedit"]').val(data.agama.flag_location);
				//$("select[name=satker] option[value="+data.agama.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.agama.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.agama.id_provinsi+"]").attr('selected','selected');
				//$("select[name=agama] option[value="+data.agama.id_jenis_agama+"]").attr('selected','selected');

				$("#satkerEdit").val(data.agama.id_satker);
				$("#provinsiEdit").val(data.agama.id_provinsi);
				$("#agamaEdit").val(data.agama.id_jenis_agama);
				...
				$("#agamaEdit").trigger('change');

				// getProvinsi(data.agama.id_provinsi)
				// getKabupaten(data.agama.id_provinsi,data.agama.id_kabupaten)
				// getKecamatan(data.agama.id_kabupaten,data.agama.id_kecamatan)
				// getKelurahan(data.agama.id_kecamatan,data.agama.id_kelurahan)

				if(data.agama.flag_location == 'prov')
				{
					getProvinsi(data.agama.id_provinsi)
					getKabupaten(data.agama.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.agama.flag_location == 'kab')
				{
					getProvinsi(data.agama.id_provinsi)
					getKabupaten(data.agama.id_provinsi,data.agama.id_kabupaten)
					getKecamatan(data.agama.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.agama.flag_location == 'kec')
				{
					getProvinsi(data.agama.id_provinsi)
					getKabupaten(data.agama.id_provinsi,data.agama.id_kabupaten)
					getKecamatan(data.agama.id_kabupaten,data.agama.id_kecamatan)
					getKelurahan(data.agama.id_kecamatan,0)
				}
				else if(data.agama.flag_location == 'kel')
				{
					getProvinsi(data.agama.id_provinsi)
					getKabupaten(data.agama.id_provinsi,data.agama.id_kabupaten)
					getKecamatan(data.agama.id_kabupaten,data.agama.id_kecamatan)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>demografi_agama/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
