<div class="section">
	...
	<div class="page-header">
		...
		<li class="breadcrumb-item"><a href="#"><i class="ti-package mr-1"></i>Demografi</a></li>
			<li class="breadcrumb-item active" aria-current="page">Desa Pesisir</li>
		</ol>
	</div>
	<!-- Page-header closed -->
		...
		<div class="col-md-12 col-lg-12">
			<div class="card" style="overflow:auto;">
				<div class="card-header">
					<div class="card-title">Desa Pesisir</div>
				...
				<div class="card-body">
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataPesisir as $pesisir): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('DEMO','update')): ?>
										<button onclick="editModal(`<?= encrypt($pesisir->id_desa_pesisir); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($pesisir->id_desa_pesisir); ?>`,'<?= $pesisir->nama_desa ?>')"
									...
									<td><?= $no++ ?></td>
									<td><?= $pesisir->nama_satker ?></td>
									<td><?= $pesisir->provinsi ?></td>
									<td><?= $pesisir->kabupaten ?></td>
									<td><?= $pesisir->kecamatan ?></td>
									<td><?= $pesisir->kelurahan ?></td>
									<td><?= $pesisir->nama_desa ?></td>
									<td><?= $pesisir->jumlah_penduduk ?></td>
									<td><?= $pesisir->tingkat_pendidikan ?></td>
									<td><?= $pesisir->nama_pembina ?></td>
									<td><?= $pesisir->nama_tertua_desa ?></td>
									<td><?= $pesisir->keterangan ?></td>
									<td><?= $pesisir->nama_pegawai ?></td>
									<td><?= $pesisir->LastUpdated ?></td>
								</tr>
								<?php endforeach ?>
							</tbody>
						</table>
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/demografi_desaPesisir/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>demografi_desaPesisirs/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>demografi_desaPesisir/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('input[name="satker"]').val(data.pesisir.id_satker);
				$('input[name="nama_desa"]').val(data.pesisir.nama_desa);
				$('input[name="jumlah_penduduk"]').val(data.pesisir.jumlah_penduduk);
				$('input[name="tingkat_pendidikan"]').val(data.pesisir.tingkat_pendidikan);
				$('input[name="nama_pembina"]').val(data.pesisir.nama_pembina);
				$('input[name="nama_tertua_desa"]').val(data.pesisir.nama_tertua_desa);
				$('input[name="notes"]').val(data.pesisir.keterangan);
				$('input[name="flag_locationedit"]').val(data.pesisir.flag_location);
				$('input[name="idprovedit"]').val(data.pesisir.id_prov);
				$('input[name="idkabedit"]').val(data.pesisir.id_kab);
				$('input[name="idkecedit"]').val(data.pesisir.id_kec);
				$('input[name="idkeledit"]').val(data.pesisir.id_kel);
				//$("select[name=satker] option[value="+data.pesisir.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.pesisir.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.pesisir.id_provinsi+"]").attr('selected','selected');

				$("#satkerEdit").val(data.pesisir.id_satker);
				$("#provinsiEdit").val(data.pesisir.id_provinsi);
				...
				// getProvinsi(data.pesisir.id_provinsi)
				// getKabupaten(data.pesisir.id_provinsi,data.pesisir.id_kabupaten)
				// getKecamatan(data.pesisir.id_kabupaten,data.pesisir.id_kecamatan)
				// getKelurahan(data.pesisir.id_kecamatan,data.pesisir.id_kelurahan)

				if(data.pesisir.flag_location == 'prov')
				{
					getProvinsi(data.pesisir.id_provinsi)
					getKabupaten(data.pesisir.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.pesisir.flag_location == 'kab')
				{
					getProvinsi(data.pesisir.id_provinsi)
					getKabupaten(data.pesisir.id_provinsi,data.pesisir.id_kabupaten)
					getKecamatan(data.pesisir.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.pesisir.flag_location == 'kec')
				{
					getProvinsi(data.pesisir.id_provinsi)
					getKabupaten(data.pesisir.id_provinsi,data.pesisir.id_kabupaten)
					getKecamatan(data.pesisir.id_kabupaten,data.pesisir.id_kecamatan)
					getKelurahan(data.pesisir.id_kecamatan,0)
				}
				else if(data.pesisir.flag_location == 'kel')
				{
					getProvinsi(data.pesisir.id_provinsi)
					getKabupaten(data.pesisir.id_provinsi,data.pesisir.id_kabupaten)
					getKecamatan(data.pesisir.id_kabupaten,data.pesisir.id_kecamatan)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>demografi_desaPesisir/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
