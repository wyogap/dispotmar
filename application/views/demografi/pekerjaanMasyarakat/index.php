<div class="section">
	...
	<div class="page-header">
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active" aria-current="page">Pekerjaan Masyarakat</li>
		</ol>
	</div>
	<!-- Page-header closed -->
		...
		<div class="col-md-12 col-lg-12">
			<div class="card" style="overflow:auto;">
				<div class="card-header">
					<div class="card-title">Pekerjaan Masyarakat</div>
				...
				<div class="card-body">
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataPekerjaan as $pekerjaan): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('DEMO','update')): ?>
										<button onclick="editModal(`<?= encrypt($pekerjaan->id_pekerjaan_masyarakat); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($pekerjaan->id_pekerjaan_masyarakat); ?>`,'<?= $pekerjaan->mayoritas1 ?>')"
									...
									<td><?= $no++ ?></td>
									<td><?= $pekerjaan->nama_satker ?></td>
									<td><?= $pekerjaan->wilayah ?></td>
									<td><?= $pekerjaan->mayoritas1 ?></td>
									<td><?= $pekerjaan->mayoritas2 ?></td>
									<td><?= $pekerjaan->mayoritas3 ?></td>
									<td><?= $pekerjaan->keterangan ?></td>
									<td><?= $pekerjaan->nama_pegawai ?></td>
									<td><?= $pekerjaan->LastUpdated ?></td>
								</tr>
								<?php endforeach ?>
							</tbody>
						</table>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<!-- <div class="form-group row">
								<label class="col-md-3 col-form-label" for="mayoritas1">Mayoritas 1</label>
								<div class="col-md-9">
									<input type="text" id="" name="mayoritas1" class="form-control">
									<div class="invalid-feedback warning-mayoritas1"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="mayoritas2">Mayoritas 2</label>
								<div class="col-md-9">
									<input type="text" id="" name="mayoritas2" class="form-control">
									<div class="invalid-feedback warning-mayoritas2"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="mayoritas3">Mayoritas 3</label>
								<div class="col-md-9">
									<input type="text" id="" name="mayoritas3" class="form-control">
									<div class="invalid-feedback warning-mayoritas3"></div>
								</div>
							</div> -->
							<div class="form-group row">
								<label class="col-md-3 col-form-label">Mayoritas 1</label>
								<div class="col-md-9">
									<select class="form-control" name="mayoritas1" id="mayoritas1" style="width: 100%;">
										<?php foreach($mayoritasPekerjaan as $kerja): ?>
											<option value="<?= $kerja->id_mayoritas_pekerjaan ?>"><?= $kerja->nama ?></option>
										<?php endforeach?>
									</select>
									<div class="invalid-feedback warning-mayoritas1"></div>
								</div>
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label">Mayoritas 2</label>
								<div class="col-md-9">
									<select class="form-control" name="mayoritas2" id="mayoritas2" style="width: 100%;">
										<?php foreach($mayoritasPekerjaan as $kerja): ?>
											<option value="<?= $kerja->id_mayoritas_pekerjaan ?>"><?= $kerja->nama ?></option>
										<?php endforeach?>
									</select>
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label">Mayoritas 3</label>
								<div class="col-md-9">
									<select class="form-control" name="mayoritas3" id="mayoritas3" style="width: 100%;">
										<?php foreach($mayoritasPekerjaan as $kerja): ?>
											<option value="<?= $kerja->id_mayoritas_pekerjaan ?>"><?= $kerja->nama ?></option>
										<?php endforeach?>
									</select>
					...
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label">Mayoritas 1</label>
								<div class="col-md-9">
									<select class="form-control" name="mayoritas1" id="mayoritas1Edit" style="width: 100%;">
										<?php foreach($mayoritasPekerjaan as $kerja): ?>
											<option value="<?= $kerja->id_mayoritas_pekerjaan ?>"><?= $kerja->nama ?></option>
										<?php endforeach?>
									</select>
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label">Mayoritas 2</label>
								<div class="col-md-9">
									<select class="form-control" name="mayoritas2" id="mayoritas2Edit" style="width: 100%;">
										<?php foreach($mayoritasPekerjaan as $kerja): ?>
											<option value="<?= $kerja->id_mayoritas_pekerjaan ?>"><?= $kerja->nama ?></option>
										<?php endforeach?>
									</select>
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label">Mayoritas 3</label>
								<div class="col-md-9">
									<select class="form-control" name="mayoritas3" id="mayoritas3Edit" style="width: 100%;">
										<?php foreach($mayoritasPekerjaan as $kerja): ?>
											<option value="<?= $kerja->id_mayoritas_pekerjaan ?>"><?= $kerja->nama ?></option>
										<?php endforeach?>
									</select>
...
<script>
	$(document).ready(function () {
		...
		$("#mayoritas1Edit").select2({
			dropdownParent: $('#editModal')
		});
		$("#mayoritas2Edit").select2({
			dropdownParent: $('#editModal')
		});
		$("#mayoritas3Edit").select2({
			dropdownParent: $('#editModal')
		});
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/demografi_pekerjaanMasyarakat/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>demografi_pekerjaanMasyarakats/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>demografi_pekerjaanMasyarakat/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('select[name=mayoritas1]').find('option:selected').removeAttr('selected');
				$('select[name=mayoritas2]').find('option:selected').removeAttr('selected');
				$('select[name=mayoritas3]').find('option:selected').removeAttr('selected');
				$('input[name="id"]').val(id);
				$('input[name="satker"]').val(data.pekerjaan.id_satker);
				$('input[name="notes"]').val(data.pekerjaan.keterangan);
				$('input[name="flag_locationedit"]').val(data.pekerjaan.flag_location);
				//$("select[name=satker] option[value="+data.pekerjaan.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.pekerjaan.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.pekerjaan.id_provinsi+"]").attr('selected','selected');
				//$("select[name=mayoritas1] option[value="+data.pekerjaan.id_mayoritas1+"]").attr('selected','selected');
				//$("select[name=mayoritas2] option[value="+data.pekerjaan.id_mayoritas2+"]").attr('selected','selected');
				//$("select[name=mayoritas3] option[value="+data.pekerjaan.id_mayoritas3+"]").attr('selected','selected');

				$("#satkerEdit").val(data.pekerjaan.id_satker);
				$("#provinsiEdit").val(data.pekerjaan.id_provinsi);
				$("#mayoritas1Edit").val(data.pekerjaan.id_mayoritas1);
				$("#mayoritas2Edit").val(data.pekerjaan.id_mayoritas2);
				$("#mayoritas3Edit").val(data.pekerjaan.id_mayoritas3);
				...
				$("#mayoritas1Edit").trigger('change');
				$("#mayoritas2Edit").trigger('change');
				$("#mayoritas3Edit").trigger('change');

				// getProvinsi(data.pekerjaan.id_provinsi)
				// getKabupaten(data.pekerjaan.id_provinsi,data.pekerjaan.id_kabupaten)
				// getKecamatan(data.pekerjaan.id_kabupaten,data.pekerjaan.id_kecamatan)
				// getKelurahan(data.pekerjaan.id_kecamatan,data.pekerjaan.id_kelurahan)

				if(data.pekerjaan.flag_location == 'prov')
				{
					getProvinsi(data.pekerjaan.id_provinsi)
					getKabupaten(data.pekerjaan.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.pekerjaan.flag_location == 'kab')
				{
					getProvinsi(data.pekerjaan.id_provinsi)
					getKabupaten(data.pekerjaan.id_provinsi,data.pekerjaan.id_kabupaten)
					getKecamatan(data.pekerjaan.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.pekerjaan.flag_location == 'kec')
				{
					getProvinsi(data.pekerjaan.id_provinsi)
					getKabupaten(data.pekerjaan.id_provinsi,data.pekerjaan.id_kabupaten)
					getKecamatan(data.pekerjaan.id_kabupaten,data.pekerjaan.id_kecamatan)
					getKelurahan(data.pekerjaan.id_kecamatan,0)
				}
				else if(data.pekerjaan.flag_location == 'kel')
				{
					getProvinsi(data.pekerjaan.id_provinsi)
					getKabupaten(data.pekerjaan.id_provinsi,data.pekerjaan.id_kabupaten)
					getKecamatan(data.pekerjaan.id_kabupaten,data.pekerjaan.id_kecamatan)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>demografi_pekerjaanMasyarakat/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
