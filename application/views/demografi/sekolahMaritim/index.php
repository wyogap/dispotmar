<div class="section">
	...
	<div class="page-header">
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active" aria-current="page">Sekolah Maritim</li>
		</ol>
	</div>
	<!-- Page-header closed -->
		...
		<div class="col-md-12 col-lg-12">
			<div class="card" style="overflow:auto;">
				<div class="card-header">
					<div class="card-title">Sekolah Maritim</div>
				...
				<div class="card-body">
					...
					<table id="example" style="table-layout: auto; width: 100%;" class="table table-striped  table-bordered key-buttons text-nowrap lowercaseCaption_DT">
							...
							<tbody>
								<?php $no=1; foreach($dataMaritim as $maritim): ?>
								<tr>
									<td class="text-center">
										<?php if(policy('DEMO','update')): ?>
										<button onclick="editModal(`<?= encrypt($maritim->id_sekolah_maritim); ?>`)"
										...
										<button
											onclick="deleteConfirm(`<?= encrypt($maritim->id_sekolah_maritim); ?>`,'<?= $maritim->nama_sekolah ?>')"
									...
									<td><?= $no++ ?></td>
									<td><?= $maritim->nama_satker ?></td>
									<td><?= $maritim->wilayah ?></td>
									<td><?= $maritim->nama_sekolah ?></td>
									<td><?= $maritim->jumlah_siswa ?></td>
									<td><?= $maritim->jumlah_pengajar ?></td>
									<td><?= $maritim->kerjasama_instansi ?></td>
									<td><?= $maritim->keterangan ?></td>
									<td><?= $maritim->nama_pegawai ?></td>
									<td><?= $maritim->LastUpdated ?></td>
								</tr>
								<?php endforeach ?>
							</tbody>
						</table>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12 col-md-12">
							...
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="nama_sekolah">Nama Perguruan Tinggi/SMA Sederajat</label>
								<div class="col-md-9">
									<input type="text" id="" name="nama_sekolah" class="form-control">
									<div class="invalid-feedback warning-nama_sekolah"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="jumlah_siswa">Jumlah Siswa</label>
								<div class="col-md-9">
									<input type="number" id="" name="jumlah_siswa" class="form-control">
									<div class="invalid-feedback warning-jumlah_siswa"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="jumlah_pengajar">Jumlah Pengajar</label>
								<div class="col-md-9">
									<input type="number" id="" name="jumlah_pengajar" class="form-control">
									<div class="invalid-feedback warning-jumlah_pengajar"></div>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-3 col-form-label" for="kerjasama_instansi">Kerjasama Dengan Instansi</label>
								<div class="col-md-9">
									<input type="text" id="" name="kerjasama_instansi" class="form-control">
									<div class="invalid-feedback warning-kerjasama_instansi"></div>
								</div>
							</div>
							<div class="form-group row">
...
<script>
	$(document).ready(function () {
		...
		$('#addForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>/demografi_sekolahMaritim/store",
				dataType: "json",
				data: $(this).serialize(),
		...
		$('#editForm').submit(function () {
			$.ajax({
				type: "POST",
				url: "<?= site_url() ?>demografi_sekolahMaritims/update",
				dataType: "json",
				data: $(this).serialize(),
				success: function (data) {
					if (data[0].status == 0) {
	...
	function editModal(id) {
		...
		$.ajax({
			type: 'ajax',
			method: 'GET',
			url: '<?= site_url() ?>demografi_sekolahMaritim/' + id,
			data: {
				id: id
			},
			dataType: 'json',
			success: function (data) {
				...
				$('input[name="satker"]').val(data.maritim.id_satker);
				$('input[name="nama_sekolah"]').val(data.maritim.nama_sekolah);
				$('input[name="jumlah_siswa"]').val(data.maritim.jumlah_siswa);
				$('input[name="jumlah_pengajar"]').val(data.maritim.jumlah_pengajar);
				$('input[name="kerjasama_instansi"]').val(data.maritim.kerjasama_instansi);
				$('input[name="notes"]').val(data.maritim.keterangan);
				$('input[name="flag_locationedit"]').val(data.maritim.flag_location);
				//$("select[name=satker] option[value="+data.maritim.id_satker+"]").attr('selected','selected');
				$("select[name=satkerPicked] option[value=" + data.maritim.id_satker + "]").attr('selected','selected');
				//$("select[name=provinsi] option[value="+data.maritim.id_provinsi+"]").attr('selected','selected');

				$("#satkerEdit").val(data.maritim.id_satker);
				$("#provinsiEdit").val(data.maritim.id_provinsi);
				...
				// getProvinsi(data.maritim.id_provinsi)
				// getKabupaten(data.maritim.id_provinsi,data.maritim.id_kabupaten)
				// getKecamatan(data.maritim.id_kabupaten,data.maritim.id_kecamatan)
				// getKelurahan(data.maritim.id_kecamatan,data.maritim.id_kelurahan)

				if(data.maritim.flag_location == 'prov')
				{
					getProvinsi(data.maritim.id_provinsi)
					getKabupaten(data.maritim.id_provinsi,0)
					getKecamatan(0,0)
					getKelurahan(0,0)
				}
				else if(data.maritim.flag_location == 'kab')
				{
					getProvinsi(data.maritim.id_provinsi)
					getKabupaten(data.maritim.id_provinsi,data.maritim.id_kabupaten)
					getKecamatan(data.maritim.id_kabupaten,0)
					getKelurahan(0,0)
				}
				else if(data.maritim.flag_location == 'kec')
				{
					getProvinsi(data.maritim.id_provinsi)
					getKabupaten(data.maritim.id_provinsi,data.maritim.id_kabupaten)
					getKecamatan(data.maritim.id_kabupaten,data.maritim.id_kecamatan)
					getKelurahan(data.maritim.id_kecamatan,0)
				}
				else if(data.maritim.flag_location == 'kel')
				{
					getProvinsi(data.maritim.id_provinsi)
					getKabupaten(data.maritim.id_provinsi,data.maritim.id_kabupaten)
					getKecamatan(data.maritim.id_kabupaten,data.maritim.id_kecamatan)
	...
	function deleteConfirm(id, content) {
		...
		$('#formDelete').attr('action', '<?= site_url() ?>demografi_sekolahMaritim/' + id + '/delete');
		$('#deleteModal').modal();
	}
</script>
