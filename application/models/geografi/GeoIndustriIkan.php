...
class GeoIndustriIkan extends CI_Model
{
    private $_table = "geo_industri_perikanan";

    public $id_industri_perikanan;
    ...
    public function get()
    {
		$sql = "SELECT DISTINCT
				...
				WHERE
					ikan.is_active = 1 
				GROUP BY
					ikan.id_industri_perikanan
				ORDER BY
					ikan.id_industri_perikanan DESC";
    ...
    public function list($request = null)
    {
		...
		satker.nama_satker,
		geografi.*');
		$this->db->from('geo_industri_perikanan AS ikan');
	...
	FROM
		...
		$this->db->group_by('ikan.id_industri_perikanan');
		$this->db->order_by('ikan.id_industri_perikanan','DESC');
	...
	public function getBySatkernew($id)
    {
		$sql = "SELECT DISTINCT
				...
				FROM
					...
					JOIN org_satker AS satker
					ON ikan.id_satker = satker.id_satker
					LEFT JOIN mst_jenis_ikan AS jenis_ikan
    ...
    {
        return $this->db->update($this->_table, $data, ["id_industri_perikanan" => $id]);
    }

    public function delete($id)
    {
        return $this->db->update($this->_table,['is_active' => FALSE], ["id_industri_perikanan" => $id]);
    }
}
