...
class TokohMasyarakatController extends CI_Controller {
	...
	{
		$this->data['title'] = 'Kondsos - Tokoh Masyarakat';

		if (policy('KONSOS','read')) {
			$this->data['dataTokoh'] = $this->tokoh->getdataForDatatable(['satker' => $this->session->userdata('id_satker')]);
		}else if (policy('KONSOS','read_all')){
			$this->data['dataTokoh'] = $this->tokoh->getdataForDatatable();
		}

		$this->data['satkers'] = $this->satker->get();
		$this->data['provinsi'] = $this->geo->getLevel(1);
		$this->data['agamas'] = $this->agama->get();

		$data['isi'] = $this->load->view('kondsos/tkhMasyarakat/index', $this->data, true);
	...
	public function show($id)
	{
		...
		$this->data['tokoh'] = $this->tokoh->find(decrypt($id));
	...
	public function store(){
		...
		$this->form_validation->set_rules('age', 'Usia', 'trim|integer|required');
	...
	public function delete($id=null)
    {
		...
		if ($this->tokoh->delete(decrypt($id))) {
