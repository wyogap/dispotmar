...
class PeninggalanSejarahController extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('Pengelola','pengelola');
    }

    public function index()
	{
		$this->data['title'] = 'Kondsos - Peninggalan Sejarah';

		if (policy('KONSOS','read')) {
			$this->data['dataSejarah'] = $this->sejarah->getdataForDatatable(['satker' => $this->session->userdata('id_satker')]);
		}else if (policy('KONSOS','read_all')){
			$this->data['dataSejarah'] = $this->sejarah->getdataForDatatable();
		}

		$this->data['pengelolas'] = $this->pengelola->get();
		$this->data['satkers'] = $this->satker->get();
		$this->data['provinsi'] = $this->geo->getLevel(1);

		$data['isi'] = $this->load->view('kondsos/penSejarah/index', $this->data, true);
	...
	public function show($id)
	{
		...
		$this->data['sejarah'] = $this->sejarah->find(decrypt($id));
	...
	public function store(){
		...
		$this->form_validation->set_rules('sejarah', 'Objek sejarah', 'trim|required');
		$this->form_validation->set_rules('pengelola', 'pengelola', 'trim|required');
		$this->form_validation->set_rules('koordinat', 'Titik Koordinat', 'trim|required');
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$response = [
				...
				'sejarah'			=> form_error('sejarah'),
				'pengelola'		=> form_error('pengelola'),
			...
			$data = array(
				...
				'objek_sejarah'		=> $this->input->post('sejarah'),
				'id_pengelola'		=> $this->input->post('pengelola'),
				'titik_kordinat'	=> $this->input->post('koordinat'),
	...
	public function update(){
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($this->sejarah->update($id,$data)) {
				$this->session->set_flashdata('success', 'Data anda berhasil disimpan');
				echo json_encode([['status' => 1]]);
			}
		}
	...
	public function delete($id=null)
    {
		...
		if ($this->sejarah->delete(decrypt($id))) {
