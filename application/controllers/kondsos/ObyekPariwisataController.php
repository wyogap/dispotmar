...
class ObyekPariwisataController extends CI_Controller {
	...
	{
		$this->data['title'] = 'Kondsos - Objek Pariwisata';

		if (policy('KONSOS','read')) {
			$this->data['dataPariwisata'] = $this->pariwisata->getdataForDatatable(['satker' => $this->session->userdata('id_satker')]);
		}else if (policy('KONSOS','read_all')){
			$this->data['dataPariwisata'] = $this->pariwisata->getdataForDatatable();
		}
		...
		$data['isi'] = $this->load->view('kondsos/objPariwisata/index', $this->data, true);
	...
	public function show($id)
	{
		...
		$this->data['pariwisata'] = $this->pariwisata->find(decrypt($id));
	...
	public function store(){
		...
		$this->form_validation->set_rules('pariwisata', 'Objek Pariwisata', 'trim|required');
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$response = [
				...
				'pariwisata'			=> form_error('pariwisata'),
			...
			$data = array(
				...
				'objek_pariwisata'		=> $this->input->post('pariwisata'),
			...
			if ($this->pariwisata->create($data)) {
	...
	public function update(){
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($this->pariwisata->update($id,$data)) {
				$this->session->set_flashdata('success', 'Data anda berhasil disimpan');
				echo json_encode([['status' => 1]]);
			}
		}
	...
	public function delete($id=null)
    {
		...
		if ($this->pariwisata->delete(decrypt($id))) {
