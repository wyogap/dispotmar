...
class UmkmController extends CI_Controller {
	...
	{
		$this->data['title'] = 'Kondsos - Industri UMKM';

		if (policy('KONSOS','read')) {
			$this->data['dataUmkm'] = $this->umkm->getdataForDatatable(['satker' => $this->session->userdata('id_satker')]);
		}else if (policy('KONSOS','read_all')){
			$this->data['dataUmkm'] = $this->umkm->getdataForDatatable();
		}

		$this->data['satkers'] = $this->satker->get();
		$this->data['provinsi'] = $this->geo->getLevel(1);

		$data['isi'] = $this->load->view('kondsos/indUmkm/index', $this->data, true);
	...
	public function show($id)
	{
		...
		$this->data['umkm'] = $this->umkm->find(decrypt($id));
	...
	public function delete($id=null)
    {
		...
		if ($this->umkm->delete(decrypt($id))) {
