...
class PerkebunanController extends CI_Controller {
	...
	{
		$this->data['title'] = 'Geografi - Perkebunan';

		if (policy('GEO','read')) {
			$this->data['dataPerkebunan'] = $this->perkebunan->getdataForDatatable(['satker' => $this->session->userdata('id_satker')]);
		}else if (policy('GEO','read_all')){
			$this->data['dataPerkebunan'] = $this->perkebunan->getdataForDatatable();
		}
		...
		$this->data['KomoditasPangan'] = $this->komoditas->getByCluster(4);
		$this->data['satkers'] = $this->satker->get();
		$this->data['provinsi'] = $this->geo->getLevel(1);

		$data['isi'] = $this->load->view('geografi/sdab/perkebunan/index', $this->data, true);
	...
	public function show($id)
	{
		...
		$this->data['perkebunan'] = $this->perkebunan->find(decrypt($id));
	...
	public function store(){
		...
		$this->form_validation->set_rules('area', 'Luas Perkebunan', 'trim|numeric|required');
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($this->perkebunan->create($data)) {
	...
	public function update(){
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($this->perkebunan->update($id,$data)) {
				$this->session->set_flashdata('success', 'Data anda berhasil disimpan');
				echo json_encode([['status' => 1]]);
			}
		}
	...
	public function delete($id=null)
    {
		...
		if ($this->perkebunan->delete(decrypt($id))) {
