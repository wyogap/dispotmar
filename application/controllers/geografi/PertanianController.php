...
class PertanianController extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('JenisTanaman_pertanian','komoditas');
    }

    public function index()
	{
		$this->data['title'] = 'Geografi - Pertanian';

		if (policy('GEO','read')) {
			$this->data['dataPertanian'] = $this->pertanian->getdataForDatatable(['satker' => $this->session->userdata('id_satker')]);
		}else if (policy('GEO','read_all')){
			$this->data['dataPertanian'] = $this->pertanian->getdataForDatatable();
		}
		...
		//$this->data['KomoditasPangan'] = $this->komoditas->getByCluster(1);
		$this->data['JenisTanaman_pertanian'] = $this->komoditas->get();
		$this->data['satkers'] = $this->satker->get();
		$this->data['provinsi'] = $this->geo->getLevel(1);

		$data['isi'] = $this->load->view('geografi/sdab/pertanian/index', $this->data, true);
	...
	public function show($id)
	{
		...
		$this->data['pertanian'] = $this->pertanian->find(decrypt($id));
	...
	public function store(){
		...
		$this->form_validation->set_rules('area', 'Luas pertanian', 'trim|numeric|required');
		$this->form_validation->set_rules('result', 'Hasil Panen', 'trim|numeric|required');
		$this->form_validation->set_rules('period', 'Masa Panen', 'trim|numeric|required');
	...
	public function update(){
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($this->pertanian->update($id,$data)) {
				$this->session->set_flashdata('success', 'Data anda berhasil disimpan');
				echo json_encode([['status' => 1]]);
			}
		}
	...
	public function delete($id=null)
    {
		...
		if ($this->pertanian->delete(decrypt($id))) {
