...
class GalanganKapalController extends CI_Controller {
	...
	{
		$this->data['title'] = 'Geografi - Galangan Kapal';

		if (policy('GEO','read')) {
			$this->data['dataGalanganKapal'] = $this->galanganKapal->getdataForDatatable(['satker' => $this->session->userdata('id_satker')]);
		}else if (policy('GEO','read_all')){
			$this->data['dataGalanganKapal'] = $this->galanganKapal->getdataForDatatable();
		}

		$this->data['satkers'] = $this->satker->get();
		$this->data['provinsi'] = $this->geo->getLevel(1);

		$data['isi'] = $this->load->view('geografi/injasMaritim/galanganKapal/index', $this->data, true);
	...
	public function show($id)
	{
		...
		$this->data['galanganKapal'] = $this->galanganKapal->find(decrypt($id));
	...
	public function store(){
		...
		$this->form_validation->set_rules('name', 'Nama Galangan', 'trim|required');
		$this->form_validation->set_rules('owner', 'Pemilik', 'trim|required');
		$this->form_validation->set_rules('maksGT', 'Maks GT', 'trim|numeric|required');
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$data = array(
				...
				'nama_galangan'	=> $this->input->post('name'),
				'pemilik'		=> $this->input->post('owner'),
				'maks_gt'		=> $this->input->post('maksGT'),
			...
			if ($this->galanganKapal->create($data)) {
	...
	public function update(){
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($this->galanganKapal->update($id,$data)) {
				$this->session->set_flashdata('success', 'Data anda berhasil disimpan');
				echo json_encode([['status' => 1]]);
			}
		}
	...
	public function delete($id=null)
    {
		...
		if ($this->galanganKapal->delete(decrypt($id))) {
