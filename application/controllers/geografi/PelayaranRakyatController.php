<?php
defined('BASEPATH') OR exit('No direct script access allowed');

class PelayaranRakyatController extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		in_access();
		$this->load->model('geografi/GeoPelayaranRakyat', 'pelayaran');
		$this->load->model('SatuanKerja', 'satker');
		$this->load->model('Geografi','geo');
    }

	public function show($id)
	{
		if (!policy('GEO','update')) show_404();

		$this->data['pelayaran'] = $this->pelayaran->find(decrypt($id));
		echo json_encode($this->data);
	}

	public function store(){
		if (!policy('GEO','create')) show_404();

		$this->form_validation->set_rules('satker', 'Satuan Kerja', 'trim|required');
		$this->form_validation->set_rules('nama_pelayaran', 'Nama Perusahaan', 'trim|required');
		$this->form_validation->set_rules('nama_kapal', 'Nama Kapal', 'trim|required');
		$this->form_validation->set_rules('gt_kapal', 'GT Kapal', 'trim|numeric|required');
		$this->form_validation->set_rules('rute', 'Rute', 'trim|required');
		$this->form_validation->set_rules('frekuensi_pelayaran', 'Frekuensi Pelayaran', 'trim|numeric|required');
		$this->form_validation->set_rules('maks_daya_angkut_orang', 'Maksimum Daya Angkut Orang', 'trim|required');
		$this->form_validation->set_rules('maks_daya_angkut_transportasi', 'Maksimum Daya Angkut Transportasi', 'trim|required');
		$this->form_validation->set_rules('provinsi', 'Provinsi', 'trim|required');
		$this->form_validation->set_rules('notes', 'Keterangan', 'trim');
		$this->form_validation->set_rules('flag_location', 'flag location', 'trim');

		if ($this->form_validation->run() == FALSE) {
			$status = ['status' => 0, 'csrf' => $this->security->get_csrf_hash()];
			$response = [
				'satker'		=> form_error('satker'),
				'nama_pelayaran' => form_error('nama_pelayaran'),
				'nama_kapal'	=> form_error('nama_kapal'),
				'gt_kapal' 		=> form_error('gt_kapal'),
				'rute'			=> form_error('rute'),
				'frekuensi_pelayaran'			=> form_error('frekuensi_pelayaran'),
				'maks_daya_angkut_orang'		=> form_error('maks_daya_angkut_orang'),
				'maks_daya_angkut_transportasi'	=> form_error('maks_daya_angkut_transportasi'),
				'provinsi' 		=> form_error('provinsi'),
				'notes' 		=> form_error('notes'),
				'flag_location' 		=> form_error('flag_location')
			];
			echo json_encode([$status,$response]);
		}else{
			$data = array(
				'id_satker'			=> $this->input->post('satker'),
				'nama_perusahaan'	=> $this->input->post('nama_pelayaran'),
				'nama_kapal'		=> $this->input->post('nama_kapal'),
				'gt_kapal'			=> $this->input->post('gt_kapal'),
				'rute'				=> $this->input->post('rute'),
				'frekuensi_pelayaran'			=> $this->input->post('frekuensi_pelayaran'),
				'maks_daya_angkut_orang'		=> $this->input->post('maks_daya_angkut_orang'),
				'maks_daya_angkut_transportasi'	=> $this->input->post('maks_daya_angkut_transportasi'),
				'keterangan'	=> $this->input->post('notes'),
				'flag_location'	=> $this->input->post('flag_location'),
				'is_active'		=> TRUE,
				'created_by'	=> $this->session->userdata('id_user'),
				'created_date'	=> date('Y-m-d H:i:s')
			);

			if ($this->input->post('kelurahan')) {
				$data['id_geografi'] = $this->input->post('kelurahan');
			}else if ($this->input->post('kecamatan')) {
				$data['id_geografi'] = $this->input->post('kecamatan');
			}else if ($this->input->post('kabupaten')) {
				$data['id_geografi'] = $this->input->post('kabupaten');
			}else if ($this->input->post('provinsi')) {
				$data['id_geografi'] = $this->input->post('provinsi');
			}

			if ($this->pelayaran->create($data)) {
				$this->session->set_flashdata('success', 'Data anda berhasil disimpan');
				echo json_encode([['status' => 1]]);
			}
		}
	}

	public function update(){
		if (!policy('GEO','update')) show_404();

		$this->form_validation->set_rules('satker', 'Satuan Kerja', 'trim|required');
		$this->form_validation->set_rules('nama_pelayaran', 'Nama Perusahaan', 'trim|required');
		$this->form_validation->set_rules('nama_kapal', 'Nama Kapal', 'trim|required');
		$this->form_validation->set_rules('gt_kapal', 'GT Kapal', 'trim|numeric|required');
		$this->form_validation->set_rules('rute', 'Rute', 'trim|required');
		$this->form_validation->set_rules('frekuensi_pelayaran', 'Frekuensi Pelayaran', 'trim|numeric|required');
		$this->form_validation->set_rules('maks_daya_angkut_orang', 'Maksimum Daya Angkut Orang', 'trim|required');
		$this->form_validation->set_rules('maks_daya_angkut_transportasi', 'Maksimum Daya Angkut Transportasi', 'trim|required');
		$this->form_validation->set_rules('provinsi', 'Provinsi', 'trim|required');
		$this->form_validation->set_rules('notes', 'Keterangan', 'trim');
		$this->form_validation->set_rules('flag_locationedit', 'flag location', 'trim');

		if ($this->form_validation->run() == FALSE) {
			$status = ['status' => 0, 'csrf' => $this->security->get_csrf_hash()];
			$response = [
				'satker'		=> form_error('satker'),
				'nama_pelayaran' => form_error('nama_pelayaran'),
				'nama_kapal'	=> form_error('nama_kapal'),
				'gt_kapal' 		=> form_error('gt_kapal'),
				'rute'			=> form_error('rute'),
				'frekuensi_pelayaran'			=> form_error('frekuensi_pelayaran'),
				'maks_daya_angkut_orang'		=> form_error('maks_daya_angkut_orang'),
				'maks_daya_angkut_transportasi'	=> form_error('maks_daya_angkut_transportasi'),
				'provinsi' 		=> form_error('provinsi'),
				'notes' 		=> form_error('notes'),
				'flag_locationedit' 		=> form_error('flag_locationedit')
			];
			echo json_encode([$status,$response]);
		}else{
			$data = array(
				'id_satker'			=> $this->input->post('satker'),
				'nama_perusahaan'	=> $this->input->post('nama_pelayaran'),
				'nama_kapal'		=> $this->input->post('nama_kapal'),
				'gt_kapal'			=> $this->input->post('gt_kapal'),
				'rute'				=> $this->input->post('rute'),
				'frekuensi_pelayaran'			=> $this->input->post('frekuensi_pelayaran'),
				'maks_daya_angkut_orang'		=> $this->input->post('maks_daya_angkut_orang'),
				'maks_daya_angkut_transportasi'	=> $this->input->post('maks_daya_angkut_transportasi'),
				'keterangan'	=> $this->input->post('notes'),
				'flag_location'	=> $this->input->post('flag_locationedit'),
				'updated_by'	=> $this->session->userdata('id_user'),
				'updated_date'	=> date('Y-m-d H:i:s')
			);

			if ($this->input->post('kelurahan')) {
				$data['id_geografi'] = $this->input->post('kelurahan');
			}else if ($this->input->post('kecamatan')) {
				$data['id_geografi'] = $this->input->post('kecamatan');
			}else if ($this->input->post('kabupaten')) {
				$data['id_geografi'] = $this->input->post('kabupaten');
			}else if ($this->input->post('provinsi')) {
				$data['id_geografi'] = $this->input->post('provinsi');
			}

			$id = decrypt($this->input->post('id'));

			if ($this->pelayaran->update($id,$data)) {
				$this->session->set_flashdata('success', 'Data anda berhasil disimpan');
				echo json_encode([['status' => 1]]);
			}
		}
	}

	public function delete($id=null)
    {
		if (!policy('GEO','delete')) show_404();

		if (!isset($id)) show_404();

		if ($this->pelayaran->delete(decrypt($id))) {
			$this->session->set_flashdata('success', 'Data berhasil dihapus');
			redirect_back();
		} else {
			$this->session->set_flashdata('error', 'Tidak dapat menghapus data');
			redirect_back();
		}
    }
}
