...
class PulauTerluarController extends CI_Controller {
	...
	{
		$this->data['title'] = 'Geografi - Pulau Terluar';

		if (policy('GEO','read')) {
			$this->data['dataPulau'] = $this->pulau->getdataForDatatable(['satker' => $this->session->userdata('id_satker')]);
		}else if (policy('GEO','read_all')){
			$this->data['dataPulau'] = $this->pulau->getdataForDatatable();
		}

		$this->data['satkers'] = $this->satker->get();
		$this->data['provinsi'] = $this->geo->getLevel(1);

		$data['isi'] = $this->load->view('geografi/sda/pulauTerluar/index', $this->data, true);
	...
	public function show($id)
	{
		...
		$this->data['pulau'] = $this->pulau->find(decrypt($id));
	...
	public function store(){
		...
		$this->form_validation->set_rules('name', 'Nama Pulau', 'trim|required');
		$this->form_validation->set_rules('area', 'Luas Pulau', 'trim|numeric|required');
		$this->form_validation->set_rules('population', 'Jumlah Penduduk', 'trim|required|numeric');
		$this->form_validation->set_rules('range', 'Jarak Pulau Utama', 'trim|numeric|required');
		$this->form_validation->set_rules('transportation', 'Transportasi', 'trim|required');
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$response = [
				...
				'population' 	=> form_error('population'),
				'range' 		=> form_error('range'),
				'transportation'=> form_error('transportation'),
			...
			$data = array(
				...
				'nama_pulau'	=> $this->input->post('name'),
				'luas_pulau'	=> $this->input->post('area'),
				'jumlah_penduduk'	=> $this->input->post('population'),
				'jarak_pulau_utama'	=> $this->input->post('range'),
				'transportasi'	=> $this->input->post('transportation'),
	...
	public function delete($id=null)
    {
		...
		if ($this->pulau->delete(decrypt($id))) {
