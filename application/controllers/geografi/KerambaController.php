...
class KerambaController extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('JenisIkan','ikan');
    }

    public function index()
	{
		$this->data['title'] = 'Geografi - Keramba Jaring Apung';

		if (policy('GEO','read')) {
			$this->data['dataKeramba'] = $this->keramba->getdataForDatatable(['satker' => $this->session->userdata('id_satker')]);
		}else if (policy('GEO','read_all')){
			$this->data['dataKeramba'] = $this->keramba->getdataForDatatable();
		}

		$this->data['jenisIkan'] = $this->ikan->get();
		$this->data['satkers'] = $this->satker->get();
		$this->data['provinsi'] = $this->geo->getLevel(1);

		$data['isi'] = $this->load->view('geografi/sdab/kerambaJaring/index', $this->data, true);
	...
	public function show($id)
	{
		...
		$this->data['keramba'] = $this->keramba->find(decrypt($id));
	...
	public function store(){
		...
		$this->form_validation->set_rules('type', 'Jenis keramba', 'trim|required');
		$this->form_validation->set_rules('area', 'Luas', 'trim|numeric|required');
		$this->form_validation->set_rules('tonase', 'Tonase', 'trim|numeric|required');
		$this->form_validation->set_rules('results', 'Penghasilan', 'trim|numeric|required');
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$data = array(
				...
				'penghasilan'	=> $this->input->post('results'),
	...
	public function update(){
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($this->keramba->update($id,$data)) {
				$this->session->set_flashdata('success', 'Data anda berhasil disimpan');
				echo json_encode([['status' => 1]]);
			}
		}
	...
	public function delete($id=null)
    {
		...
		if ($this->keramba->delete(decrypt($id))) {
