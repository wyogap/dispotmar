...
class SumberAirController extends CI_Controller {
	...
	{
		$this->data['title'] = 'Geografi - Sumber Air';

		if (policy('GEO','read')) {
			$this->data['dataAir'] = $this->air->getdataForDatatable(['satker' => $this->session->userdata('id_satker')]);
		}else if (policy('GEO','read_all')){
			$this->data['dataAir'] = $this->air->getdataForDatatable();
		}

		$this->data['jenisSumber'] = $this->sumber->get();
		$this->data['satkers'] = $this->satker->get();
		$this->data['provinsi'] = $this->geo->getLevel(1);

		$data['isi'] = $this->load->view('geografi/sda/sumberAir/index', $this->data, true);
	...
	public function show($id)
	{
		...
		$this->data['air'] = $this->air->find(decrypt($id));
	...
	public function store(){
		...
		$this->form_validation->set_rules('sources', 'Sumber Air', 'trim|required');
		$this->form_validation->set_rules('volume', 'Debit Air', 'trim|numeric|required');
		$this->form_validation->set_rules('condition', 'Kondisi Air', 'trim|required');
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$data = array(
				...
				'id_jenis_sumberair'	=> $this->input->post('sources'),
				'debit_air'		=> $this->input->post('volume'),
				'kondisi_air'	=> $this->input->post('condition'),
